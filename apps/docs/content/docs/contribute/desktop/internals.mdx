...
That means desktop work is usually spread across four boundaries: React renderer code, preload bridge code, Electron main-process code, and the staged runtime bundle under `apps/desktop/out/runtime-<platform>`.

## Main code seams

- `apps/desktop/src/components/layout/AppShell.tsx`: the shell composition and top-level product routing. This is where the top-toolbar shell navigation, agent pane, browser panes, file explorer, applications explorer, app surfaces, operations drawer, task-proposal toasts, settings overlays, and reported non-browser operator surfaces are coordinated.
- `apps/desktop/src/components/layout/TopTabsBar.tsx`: the shared desktop toolbar, including workspace switching and the shell-level `Space`, `Automations`, and `Marketplace` entry points.
- `apps/desktop/src/components/onboarding/FirstWorkspacePane.tsx`, `ConfigureStep.tsx`, `BrowserProfileStep.tsx`, and `CreatingView.tsx`: first-workspace flow, including empty/template workspace creation and browser-profile bootstrap choices.
- `apps/desktop/shared/model-catalog.ts`: shipped fallback model metadata for direct providers and holaOS Proxy model mapping, including reasoning support, allowed thinking values, and input modalities.
- `apps/desktop/src/lib/workspaceDesktop.tsx` and `apps/desktop/src/lib/workspaceSelection.tsx`: renderer-side workspace state and shell coordination.
- `apps/desktop/src/components/auth/AuthPanel.tsx`: provider settings, catalog-backed defaults, background-task selection, recall embeddings, and image-generation configuration.
- `apps/desktop/src/components/panes/ChatPane/index.tsx`: model picker, per-model reasoning-effort selector, queued chat inputs, and stream/timeline rendering.
- `apps/desktop/src/components/panes/BrowserPane.tsx`, `SpaceBrowserExplorerPane.tsx`, and `SpaceBrowserDisplayPane.tsx`: the browser-space UI for the `user` and `agent` browser surfaces.
- `apps/desktop/src/components/panes/FileExplorerPane.tsx`: workspace file explorer, previews, editing, bookmarking, drag-and-drop imports, and file-watch behavior.
- `apps/desktop/src/components/panes/SpaceApplicationsExplorerPane.tsx`: workspace app list for the space explorer, parallel to files and browser, including the `Add app` action that routes into the Marketplace apps tab.
- `apps/desktop/src/components/layout/NotificationToastStack.tsx`: runtime-backed notification rendering and activation behavior.
- `apps/desktop/electron/preload.ts`: the renderer-to-main bridge exposed on `window.electronAPI`.
- `apps/desktop/src/types/electron.d.ts`: the typed source of truth for the preload bridge. If the renderer can call it, it should be declared here.
- `apps/desktop/electron/main.ts`: the main-process implementation for IPC, BrowserView orchestration, embedded runtime startup, workspace actions, file system actions, and browser state.

## Embedded runtime lifecycle

The desktop does not talk to the runtime through a vague helper. `apps/desktop/electron/main.ts` owns the launch path:

1. Resolve the staged runtime bundle under `apps/desktop/out/runtime-<platform>` or an override such as `HOLABOSS_RUNTIME_ROOT`.
...
3. Spawn `bin/sandbox-runtime` with desktop-owned env such as `HB_SANDBOX_ROOT`, `SANDBOX_AGENT_BIND_HOST=127.0.0.1`, `SANDBOX_AGENT_BIND_PORT=<resolved runtime port>`, `SANDBOX_AGENT_HARNESS`, `HOLABOSS_HOST_STATE_DB_PATH`, `HOLABOSS_CONTROL_PLANE_DB_PATH`, the legacy `HOLABOSS_RUNTIME_DB_PATH` alias, and the bridge settings.
...
The embedded runtime port is not fixed anymore. `apps/desktop/electron/main.ts` resolves it from `HOLABOSS_RUNTIME_API_PORT` or, when unset, from the resolved Electron `userData` path. Port `5060` is explicitly rejected, which is why older fixed-port assumptions are no longer reliable for diagnostics.

Representative launch shape from `apps/desktop/electron/main.ts`:
...
1. `apps/desktop/electron/preload.ts`
2. `apps/desktop/src/types/electron.d.ts`
3. the `ipcMain` handler in `apps/desktop/electron/main.ts`, usually wired through `handleTrustedIpc(...)`
...
`createWorkspace()` in `apps/desktop/electron/main.ts` now also emits structured stage logs with the `[holaboss.createWorkspace]` prefix for debugging create failures across materialization, runtime record creation, template apply, and workspace activation.
...
- `apps/desktop/electron/main.ts` refreshes a managed runtime model catalog from the control plane when a signed-in holaOS session is active, caches that catalog, and merges it with the locally persisted `runtime-config.json`.
...
Use `apps/desktop/shared/model-catalog.ts` when you need the desktop to know more than a raw model id:
...
- Use `apps/desktop/electron/browser-operator-surface-context.test.mjs` and `apps/desktop/electron/runtime-quit-cleanup.test.mjs` as the fastest regression checks when you are touching browser-service context or app-quit cleanup.
- Use `bash apps/desktop/scripts/check-runtime-status.sh` when the embedded runtime fails to start, a workspace looks corrupted, or app lifecycle behavior diverges from the desktop UI. The helper now follows the same `HOLABOSS_RUNTIME_API_PORT` and `userData`-path-derived port rules as the desktop itself.
