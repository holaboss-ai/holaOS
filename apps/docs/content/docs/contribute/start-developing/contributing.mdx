...
| --- | --- | --- |
| Desktop shell, renderer, preload, or Electron main process | `apps/desktop/src/**`, `apps/desktop/electron/**` | `npm run desktop:typecheck`; also run `npm run desktop:e2e` when browser panes, IPC, or runtime launch behavior changes |
...
| SDK packages | `sdk/app-sdk/**`, `sdk/app-builder-sdk/**`, `sdk/ui/**` | Run the matching `build`, `typecheck`, and `test` scripts for the package you changed, for example `bun --filter=@holaboss/app-builder-sdk run typecheck` and `bun --filter=@holaboss/app-builder-sdk run test` |
| Docs app and content | `website/docs/**` | `npm run docs:typecheck` and `npm run docs:build` |

## Cross-cutting rules

- If you change the Electron bridge, update all of `apps/desktop/electron/preload.ts`, `apps/desktop/src/types/electron.d.ts`, and the `ipcMain` handler in `apps/desktop/electron/main.ts`.
...
- If you change runtime bundle layout, launcher behavior, or required packaged files, update both `runtime/deploy/**` and the desktop staging logic in `apps/desktop/scripts/runtime-bundle*.mjs`.
...
- Validation: npm run desktop:typecheck; npm run runtime:test; npm run docs:typecheck; npm run docs:build
```
