...
`createWorkspace()` in `apps/desktop/electron/main.ts` supports four modes:
...
That is the path to use when you need a versioned and shareable template source instead of a local template folder.
...
- local template folders must always contain `workspace.yaml`
