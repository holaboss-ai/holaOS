...
- workspace-scoped interaction memory still lives under each workspace's `.holaboss/memory/interaction/`, with derived semantic trees under `.holaboss/memory/semantic/interaction/trees/`
- runtime continuity state remains separate from durable memory under each workspace's `.holaboss/memory/runtime/`
