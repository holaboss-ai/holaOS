...
This page explains why the app layer exists and how it fits into the system. For manifest fields, lifecycle hooks, MCP registration, and output-publishing behavior, continue into the [Build > Apps](/docs/build/apps) pages.
...
`Apps` and `Plugins` are separate systems in `holaOS`.

- `Apps` are runtime-managed capability packages declared in `workspace.yaml` under `applications[]`.
- `Plugins` are workspace-native structural units that own one base plus plugin-owned workflows and custom teammates.
- a plugin's base owns that plugin's dashboards and objects

That means a workspace can install apps and also contain plugins, but the runtime does not treat them as the same thing:

- apps are declared through authored manifest paths under `apps/`
- plugins are stored as workspace runtime structure and surfaced directly in the desktop shell
...
<DocCards>
  ...
  <DocCard
    ...
    href="/docs/build/apps/app-builder-sdk"
    description="See the TypeScript SDK new apps build against — five primitives, typed integration calls, automatic MCP surfacing."
  />
</DocCards>
