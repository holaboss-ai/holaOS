...
<DocDefinition term="App Builder SDK" meta="app authoring layer">
  The App Builder SDK is the TypeScript SDK new Holaboss workspace apps build against. It exposes five orthogonal primitives — `connection`, `resource`, `action`, `sync`, `start` — that compose into any app shape, normalizes upstream HTTP errors into typed `BridgeError` codes, and auto-derives the MCP tool surface from declared actions. The package is `@holaboss/app-builder-sdk`.
</DocDefinition>
