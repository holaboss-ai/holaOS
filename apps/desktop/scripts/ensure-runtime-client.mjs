...
const sdkRoot = path.resolve(desktopRoot, "..", "..", "packages", "runtime-client");
...
if (!outputsExist || outputsStale) {
  console.log(
    outputsExist
      ? "[ensure-runtime-client] packages/runtime-client build is stale; rebuilding."
      : "[ensure-runtime-client] packages/runtime-client build output missing; building.",
  );
  runNpm(["run", "build"], {
    cwd: sdkRoot,
    stdio: "inherit",
