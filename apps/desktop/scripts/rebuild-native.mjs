...
// Native modules rebuilt against the Electron ABI. node-mac-permissions backs
// the macOS privacy-permission triggers (CGRequestScreenCaptureAccess etc.) in
// the main process; better-sqlite3 backs the runtime DB.
const NATIVE_MODULES = ["better-sqlite3", "node-mac-permissions"];
...
function resignNativeBindings() {
  ...
  for (const moduleName of NATIVE_MODULES) {
    ...
    for (const entry of fs.readdirSync(moduleDir)) {
      ...
      execFileSync("codesign", ["--force", "--sign", "-", realPath], {
        stdio: "inherit",
      });
    }
  }
