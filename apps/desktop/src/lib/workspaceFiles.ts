...
export interface ListWorkspaceFilesOptions {
  ...
  signal?: AbortSignal;
  /**
   * Override the walk's root. When provided, the BFS starts at this absolute
   * path instead of the workspace's own root. Used to scope the `@`-mention
   * picker to the active session's project directory: project-bound chats
   * walk under `<project_path>`, General chats fall back to the workspace
   * root by leaving this undefined.
   */
...
export async function listWorkspaceFiles(
  ...
  // referenced) files first when truncated by maxFiles. When rootAbsolutePath
  // is supplied, seed the walk there; otherwise the IPC's null-path branch
  // resolves to the workspace's own root.
  const queue: Array<{
    ...
    {
      absolutePath:
        rootAbsolutePath && rootAbsolutePath.trim().length > 0
          ? rootAbsolutePath
          : null,
      relPath: "",
      depth: 0,
  ...
  // With an explicit absolute walk root (project_path override) we already know
  // the OS path, so pass a null workspaceId and let the main-side resolver use
  // it directly. With no root we must pass the workspaceId so the handler's
  // null-path branch resolves to the workspace's own root.
  const useWorkspaceScope = !rootAbsolutePath;
  ...
  while (queue.length > 0 && collected.length < maxFiles) {
    ...
    try {
      response = await window.electronAPI.fs.listDirectory(
        next.absolutePath,
        useWorkspaceScope ? workspaceId : null,
      );
    } catch {
      continue;
    }
