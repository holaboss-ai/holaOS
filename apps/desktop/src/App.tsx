...
import { RewardCompletionNotifier } from "@/components/layout/shell/RewardCompletionNotifier";
    ...
    <ErrorBoundary>
      <QueryClientProvider client={queryClient}>
        <MotionConfig reducedMotion="user">
          <TooltipProvider>
            {/* Mounted outside RequireAuth so the frameless Windows window
                always has minimize/restore/close — including on the boot
                splash and sign-in gate. No-op on macOS/Linux. */}
            <WindowsTitlebarControls />
            <RequireAuth>
              <UmamiIdentity />
              <AppShell />
              <RewardCompletionNotifier />
            </RequireAuth>
          </TooltipProvider>
        </MotionConfig>
        {/*
