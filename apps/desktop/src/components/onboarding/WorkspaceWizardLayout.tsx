...
interface WorkspaceWizardLayoutProps {
  /** Step counter. Omit on single-step surfaces to hide the indicator. */
  stepIndex?: number;
  /** Total steps. Omit (or pass 1) on single-step surfaces. */
  stepTotal?: number;
  title: string;
  description?: string;
...
export function WorkspaceWizardLayout({
  ...
  return (
    <div className="flex w-full flex-1 items-center justify-center px-5 py-8">
      ...
      >
        {/* Step counter — short dashes, using our tokens.
            Hidden on single-step surfaces (no stepTotal or stepTotal === 1). */}
        {stepIndex !== undefined && stepTotal !== undefined && stepTotal > 1 ? (
