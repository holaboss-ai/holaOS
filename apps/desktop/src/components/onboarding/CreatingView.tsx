...
export function CreatingView({
  ...
  // The step animation above is cosmetic — it has no real progress signal,
  // so surface a "taking longer than usual" hint once creation overruns,
  // rather than sitting forever on a finished-looking last step.
  useEffect(() => {
    setTookTooLong(false);
    if (workspaceCreatePhase !== "creating_workspace") {
      return;
    }
    const timer = setTimeout(() => setTookTooLong(true), 30_000);
    ...
    <section className={cn(sectionClassName, "grid place-items-center")}>
      <div className="w-full max-w-[440px] px-5 pb-8">
        <div className="rounded-2xl bg-background px-9 pt-12 pb-9 shadow-xs">
          ...
          {/* Summary rows — one per step: icon block on the left, label + status
              text in the middle, status indicator on the right. */}
          ...
          {tookTooLong ? (
            <p className="mx-auto mt-4 max-w-[320px] text-center text-xs leading-relaxed text-muted-foreground/80">
              This is taking longer than usual. You can keep waiting, or close
              this and try again.
            </p>
          ) : null}
        </div>
