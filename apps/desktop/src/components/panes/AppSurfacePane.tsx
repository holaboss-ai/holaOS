...
export function AppSurfacePane({
  ...
  // provider_id used to come from the marketplace appCatalog; the
  // installed-app payload doesn't carry it yet, so AppIcon + display
  // gracefully degrade to letter / local-svg fallback.
          ...
          <>
            {frameUrl ? (
              <iframe
                ...
                onError={() => {
                  setFrameLoading(false);
                  setFrameError("The app preview failed to load.");
                }}
              />
            ) : null}
            {frameLoading ? (
