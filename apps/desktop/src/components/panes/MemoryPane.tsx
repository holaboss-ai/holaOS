import {
  BrainCircuit,
  ChevronDown,
  ChevronRight,
  FileText,
  FolderOpen,
  Loader2,
  MoreHorizontal,
  RefreshCw,
  Search,
} from "@/components/ui/icons";
import { useCallback, useEffect, useMemo, useState } from "react";

import { remoteApi } from "@/lib/remoteApiClient";
import { SettingsCard } from "@/components/settings";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuTrigger,
} from "@/components/ui/dropdown-menu";
import { EmptyState } from "@/components/ui/empty-state";
import { cn } from "@/lib/utils";
import { useWorkspaceDesktop } from "@/lib/workspaceDesktop";
import {
  buildRelatedTargetResolution,
  findGraphNodeForPath,
  isNavigableMemoryRelationTarget,
  parseMemoryRelatedSections,
  resolveParsedRelatedEntities,
  resolveParsedRelatedRelations,
} from "@/components/panes/memoryPaneModel";

type MemoryViewMode = "file" | "graph";

const GRAPH_LAYOUT_MIN_WIDTH = 860;
const GRAPH_LAYOUT_MIN_HEIGHT = 620;
const GRAPH_LAYOUT_PADDING = 56;
const GRAPH_LAYER_GAP = 96;
const GRAPH_SIBLING_GAP = 34;
const MEMORY_GRAPH_MAX_LAYERS = 6;
const MEMORY_GRAPH_MAX_NODES = 320;

function normalizeErrorMessage(error: unknown): string {
  return error instanceof Error ? error.message : "Request failed.";
}

function firstFilePath(
  node: MemoryBrowserTreeNodePayload | null | undefined,
): string | null {
  if (!node) {
    return null;
  }
  if (node.kind === "file") {
    return node.path;
  }
  for (const child of node.children ?? []) {
    const found = firstFilePath(child);
    if (found) {
      return found;
    }
  }
  return null;
}

function treeHasFile(
  node: MemoryBrowserTreeNodePayload | null | undefined,
  targetPath: string,
): boolean {
  if (!node) {
    return false;
  }
  if (node.kind === "file") {
    return node.path === targetPath;
  }
  return (node.children ?? []).some((child) => treeHasFile(child, targetPath));
}

function ancestorPaths(targetPath: string): string[] {
  const segments = targetPath.split("/").filter(Boolean);
  const paths = [""];
  let current = "";
  for (const segment of segments.slice(0, -1)) {
    current = current ? `${current}/${segment}` : segment;
    paths.push(current);
  }
  return paths;
}

function formatBytes(bytes: number): string {
  if (!Number.isFinite(bytes) || bytes <= 0) {
    return "0 B";
  }
  const units = ["B", "KB", "MB", "GB"];
  let value = bytes;
  let unitIndex = 0;
  while (value >= 1024 && unitIndex < units.length - 1) {
    value /= 1024;
    unitIndex += 1;
  }
  const rounded =
    value >= 10 || unitIndex === 0
      ? Math.round(value)
      : Math.round(value * 10) / 10;
  return `${rounded} ${units[unitIndex]}`;
}

function countMemoryFiles(node: MemoryBrowserTreeNodePayload | null): number {
  if (!node) return 0;
  if (node.kind === "file") return 1;
  let total = 0;
  for (const child of node.children ?? []) {
    total += countMemoryFiles(child);
  }
  return total;
}

function filterMemoryTreeByQuery(
  node: MemoryBrowserTreeNodePayload,
  query: string,
): MemoryBrowserTreeNodePayload | null {
  const haystack = `${node.name} ${node.path}`.toLowerCase();
  if (node.kind === "file") {
    return haystack.includes(query) ? node : null;
  }
  const filteredChildren = (node.children ?? [])
    .map((child) => filterMemoryTreeByQuery(child, query))
    .filter((child): child is MemoryBrowserTreeNodePayload => child != null);
  if (filteredChildren.length === 0 && !haystack.includes(query)) {
    return null;
  }
  return { ...node, children: filteredChildren };
}

function relationResolutionBadgeClass(
  kind: MemoryBrowserNodeRelationPayload["target_resolution_kind"],
): string {
  if (kind === "resolved") {
    return "border-emerald-500/30 bg-emerald-500/10 text-emerald-700";
  }
  if (kind === "missing") {
    return "border-amber-500/30 bg-amber-500/10 text-amber-700";
  }
  return "border-slate-500/30 bg-slate-500/10 text-slate-700";
}

function relationResolutionLabel(
  kind: MemoryBrowserNodeRelationPayload["target_resolution_kind"],
): string {
  return kind === "resolved"
    ? "Resolved"
    : kind === "missing"
      ? "Missing"
      : "Synthetic";
}

function defaultExpandedPaths(): Set<string> {
  return new Set([
    "",
    "workspace",
    "workspace/systems",
    "workspace/systems/integrations",
  ]);
}

function graphNodeColor(node: MemoryBrowserGraphNodePayload): string {
  if (node.kind === "root") {
    return "#f2f4f8";
  }
  if (node.kind === "section") {
    return "#2dd4bf";
  }
  if (node.kind === "tree") {
    return "#f59e0b";
  }
  if (node.kind === "node") {
    return "#60a5fa";
  }
  if (node.kind === "summary") {
    return "#94a3b8";
  }
  return "#d1d5db";
}

function graphNodeFill(node: MemoryBrowserGraphNodePayload): string {
  if (node.kind === "root") {
    return "#f8fafc";
  }
  if (node.kind === "section") {
    return "#14b8a6";
  }
  if (node.kind === "tree") {
    return "#f59e0b";
  }
  if (node.kind === "node") {
    return "#3b82f6";
  }
  if (node.kind === "summary") {
    return "#d1d5db";
  }
  return "#9ca3af";
}

function graphNodeRadius(node: MemoryBrowserGraphNodePayload): number {
  if (node.kind === "root") {
    return 11;
  }
  if (node.kind === "section") {
    return 8.5;
  }
  if (node.kind === "tree") {
    return 7.5;
  }
  if (node.kind === "node") {
    return 4.7;
  }
  if (node.kind === "summary") {
    return 4.25;
  }
  return 3.1;
}

function nodeSortKey(node: MemoryBrowserGraphNodePayload): string {
  const kindOrder =
    node.kind === "root"
      ? "0"
      : node.kind === "section"
        ? "1"
        : node.kind === "tree"
          ? "2"
          : node.kind === "node"
            ? "3"
            : node.kind === "summary"
              ? "4"
              : "5";
  const level = node.level == null ? "999" : String(node.level).padStart(3, "0");
  return `${kindOrder}:${level}:${node.label.toLowerCase()}`;
}

interface PositionedGraphNode extends MemoryBrowserGraphNodePayload {
  x: number;
  y: number;
  radius: number;
}

interface MemoryGraphLayout {
  width: number;
  height: number;
  nodes: PositionedGraphNode[];
}

interface MemoryGraphViewBox {
  minX: number;
  minY: number;
  width: number;
  height: number;
}

function hashValue(value: string): number {
  let hash = 2166136261;
  for (let index = 0; index < value.length; index += 1) {
    hash ^= value.charCodeAt(index);
    hash = Math.imul(hash, 16777619);
  }
  return hash >>> 0;
}

function seededUnit(value: string, salt: string): number {
  return (hashValue(`${salt}:${value}`) % 10_000) / 10_000;
}

function buildGraphLayout(
  nodes: MemoryBrowserGraphNodePayload[],
  edges: MemoryBrowserGraphEdgePayload[],
): MemoryGraphLayout {
  if (nodes.length === 0) {
    return { width: GRAPH_LAYOUT_MIN_WIDTH, height: GRAPH_LAYOUT_MIN_HEIGHT, nodes: [] };
  }

  const width = Math.max(
    GRAPH_LAYOUT_MIN_WIDTH,
    520 + Math.sqrt(nodes.length) * 170,
  );
  const height = Math.max(
    GRAPH_LAYOUT_MIN_HEIGHT,
    420 + Math.sqrt(nodes.length) * 150,
  );
  const centerX = width / 2;
  const centerY = height / 2;

  const degreeById = new Map<string, number>();
  for (const node of nodes) {
    degreeById.set(node.id, 0);
  }
  for (const edge of edges) {
    degreeById.set(edge.from, (degreeById.get(edge.from) ?? 0) + 1);
    degreeById.set(edge.to, (degreeById.get(edge.to) ?? 0) + 1);
  }

  const byDepth = [...nodes].sort(
    (left, right) =>
      (left.level ?? (left.kind === "root" ? 0 : 10)) -
        (right.level ?? (right.kind === "root" ? 0 : 10)) ||
      nodeSortKey(left).localeCompare(nodeSortKey(right)),
  );
  const positioned = byDepth.map((node, index) => {
    const radius = graphNodeRadius(node);
    const ring =
      node.kind === "root"
        ? 0
        : node.kind === "section"
          ? 1
          : node.kind === "tree"
            ? 2
            : node.kind === "node"
              ? 3
              : node.kind === "summary"
                ? 4
                : 5;
    const angle = seededUnit(node.id, "angle") * Math.PI * 2;
    const spread = 68 + ring * 92 + seededUnit(node.id, "spread") * 32;
    const jitterX = (seededUnit(node.id, "jx") - 0.5) * 34;
    const jitterY = (seededUnit(node.id, "jy") - 0.5) * 34;
    return {
      ...node,
      radius,
      x:
        node.kind === "root"
          ? centerX
          : centerX + Math.cos(angle) * spread + jitterX + (index % 3) * 2,
      y:
        node.kind === "root"
          ? centerY
          : centerY + Math.sin(angle) * spread + jitterY + (index % 5) * 1.5,
    };
  });

  const positionById = new Map(positioned.map((node) => [node.id, node]));
  const iterations = nodes.length > 180 ? 160 : 220;
  const repulsionBase = nodes.length > 180 ? 8500 : 11000;
  const centerForce = 0.0024;
  const edgeSpring = 0.0105;

  for (let iteration = 0; iteration < iterations; iteration += 1) {
    const velocities = new Map<string, { x: number; y: number }>();
    for (const node of positioned) {
      velocities.set(node.id, { x: 0, y: 0 });
    }

    for (let leftIndex = 0; leftIndex < positioned.length; leftIndex += 1) {
      const left = positioned[leftIndex];
      for (let rightIndex = leftIndex + 1; rightIndex < positioned.length; rightIndex += 1) {
        const right = positioned[rightIndex];
        let dx = right.x - left.x;
        let dy = right.y - left.y;
        let distanceSq = dx * dx + dy * dy;
        if (distanceSq < 0.01) {
          dx = 0.1 + seededUnit(`${left.id}:${right.id}`, "dx");
          dy = 0.1 + seededUnit(`${left.id}:${right.id}`, "dy");
          distanceSq = dx * dx + dy * dy;
        }
        const distance = Math.sqrt(distanceSq);
        const minDistance = left.radius + right.radius + 18;
        const repulsion = repulsionBase / Math.max(distanceSq, 120);
        const overlap = Math.max(0, minDistance - distance) * 0.16;
        const force = repulsion + overlap;
        const fx = (dx / distance) * force;
        const fy = (dy / distance) * force;
        const leftVelocity = velocities.get(left.id);
        const rightVelocity = velocities.get(right.id);
        if (leftVelocity) {
          leftVelocity.x -= fx;
          leftVelocity.y -= fy;
        }
        if (rightVelocity) {
          rightVelocity.x += fx;
          rightVelocity.y += fy;
        }
      }
    }

    for (const edge of edges) {
      const from = positionById.get(edge.from);
      const to = positionById.get(edge.to);
      if (!from || !to) {
        continue;
      }
      const dx = to.x - from.x;
      const dy = to.y - from.y;
      const distance = Math.max(1, Math.sqrt(dx * dx + dy * dy));
      const targetDistance =
        from.kind === "root" || to.kind === "root"
          ? 146
          : from.kind === "section" || to.kind === "section"
            ? 124
            : from.kind === "tree" || to.kind === "tree"
            ? 106
            : from.kind === "node" || to.kind === "node"
              ? 82
              : from.kind === "summary" || to.kind === "summary"
                ? 74
                : 68;
      const spring = (distance - targetDistance) * edgeSpring;
      const fx = (dx / distance) * spring;
      const fy = (dy / distance) * spring;
      const fromVelocity = velocities.get(from.id);
      const toVelocity = velocities.get(to.id);
      if (fromVelocity) {
        fromVelocity.x += fx;
        fromVelocity.y += fy;
      }
      if (toVelocity) {
        toVelocity.x -= fx;
        toVelocity.y -= fy;
      }
    }

    for (const node of positioned) {
      if (node.kind === "root") {
        node.x = centerX;
        node.y = centerY;
        continue;
      }
      const velocity = velocities.get(node.id);
      if (!velocity) {
        continue;
      }
      const degree = degreeById.get(node.id) ?? 1;
      const centralBias =
        node.kind === "section"
          ? 0.48
          : node.kind === "tree"
            ? 0.4
            : node.kind === "node"
              ? 0.24
              : node.kind === "summary"
                ? 0.18
                : 0.07;
      velocity.x += (centerX - node.x) * (centerForce + centralBias / 120);
      velocity.y += (centerY - node.y) * (centerForce + centralBias / 120);
      const dampedX = velocity.x / Math.max(1, Math.sqrt(degree));
      const dampedY = velocity.y / Math.max(1, Math.sqrt(degree));
      node.x += Math.max(-14, Math.min(14, dampedX));
      node.y += Math.max(-14, Math.min(14, dampedY));
      node.x = Math.max(
        GRAPH_LAYOUT_PADDING,
        Math.min(width - GRAPH_LAYOUT_PADDING, node.x),
      );
      node.y = Math.max(
        GRAPH_LAYOUT_PADDING,
        Math.min(height - GRAPH_LAYOUT_PADDING, node.y),
      );
    }
  }

  return { width, height, nodes: positioned };
}

function buildGraphViewBox(nodes: PositionedGraphNode[]): MemoryGraphViewBox {
  if (nodes.length === 0) {
    return {
      minX: 0,
      minY: 0,
      width: GRAPH_LAYOUT_MIN_WIDTH,
      height: GRAPH_LAYOUT_MIN_HEIGHT,
    };
  }

  let minX = Number.POSITIVE_INFINITY;
  let minY = Number.POSITIVE_INFINITY;
  let maxX = Number.NEGATIVE_INFINITY;
  let maxY = Number.NEGATIVE_INFINITY;

  for (const node of nodes) {
    const halo = node.radius + 14;
    minX = Math.min(minX, node.x - halo);
    minY = Math.min(minY, node.y - halo);
    maxX = Math.max(maxX, node.x + halo);
    maxY = Math.max(maxY, node.y + halo);
  }

  const contentWidth = Math.max(120, maxX - minX);
  const contentHeight = Math.max(120, maxY - minY);
  const padding = Math.max(28, Math.min(contentWidth, contentHeight) * 0.12);

  return {
    minX: minX - padding,
    minY: minY - padding,
    width: contentWidth + padding * 2,
    height: contentHeight + padding * 2,
  };
}

interface MemoryTreeEntryProps {
  node: MemoryBrowserTreeNodePayload;
  depth: number;
  expandedPaths: Set<string>;
  selectedPath: string | null;
  onToggleDirectory: (targetPath: string) => void;
  onSelectFile: (targetPath: string) => void;
}

function MemoryTreeEntry({
  node,
  depth,
  expandedPaths,
  selectedPath,
  onToggleDirectory,
  onSelectFile,
}: MemoryTreeEntryProps) {
  const indentStyle = { paddingLeft: `${depth * 14}px` };
  if (node.kind === "directory") {
    const expanded = expandedPaths.has(node.path);
    return (
      <div className="grid gap-1">
        <button
          type="button"
          onClick={() => onToggleDirectory(node.path)}
          className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm text-foreground transition-colors hover:bg-muted"
          style={indentStyle}
        >
          {expanded ? (
            <ChevronDown className="size-3.5 text-muted-foreground" />
          ) : (
            <ChevronRight className="size-3.5 text-muted-foreground" />
          )}
          <FolderOpen className="size-4 text-muted-foreground" />
          <span className="min-w-0 flex-1 truncate">{node.name}</span>
          <span className="text-[11px] text-muted-foreground">
            {(node.children ?? []).length}
          </span>
        </button>
        {expanded
          ? (node.children ?? []).map((child) => (
              <MemoryTreeEntry
                key={child.path || child.name}
                node={child}
                depth={depth + 1}
                expandedPaths={expandedPaths}
                selectedPath={selectedPath}
                onToggleDirectory={onToggleDirectory}
                onSelectFile={onSelectFile}
              />
            ))
          : null}
      </div>
    );
  }

  const selected = selectedPath === node.path;
  return (
    <button
      type="button"
      onClick={() => onSelectFile(node.path)}
      className={`flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm transition-colors ${
        selected
          ? "bg-primary/10 text-foreground"
          : "text-muted-foreground hover:bg-muted hover:text-foreground"
      }`}
      style={indentStyle}
    >
      <FileText className="size-4" />
      <span className="min-w-0 flex-1 truncate">{node.name}</span>
    </button>
  );
}

interface MemoryGraphCanvasProps {
  graph: MemoryBrowserGraphResponsePayload;
  selectedNodeId: string | null;
  onSelectNode: (node: MemoryBrowserGraphNodePayload) => void;
}

function MemoryGraphCanvas({
  graph,
  selectedNodeId,
  onSelectNode,
}: MemoryGraphCanvasProps) {
  const hasRenderableContent = useMemo(
    () => graph.nodes.some((node) => node.kind !== "root"),
    [graph.nodes],
  );
  const layout = useMemo(
    () => buildGraphLayout(graph.nodes, graph.edges),
    [graph.edges, graph.nodes],
  );
  const positionedById = useMemo(
    () => new Map(layout.nodes.map((node) => [node.id, node])),
    [layout.nodes],
  );
  const viewBox = useMemo(() => buildGraphViewBox(layout.nodes), [layout.nodes]);

  if (!hasRenderableContent) {
    return (
      <div className="flex h-full items-center justify-center p-4 text-sm text-muted-foreground">
        No graph nodes are visible for this selection.
      </div>
    );
  }

  return (
    <div className="h-full overflow-hidden bg-[#171717]">
      <svg
        viewBox={`${viewBox.minX} ${viewBox.minY} ${viewBox.width} ${viewBox.height}`}
        className="block h-full w-full"
        preserveAspectRatio="xMidYMid meet"
        style={{
          minHeight: "100%",
        }}
      >
        <rect
          x={viewBox.minX}
          y={viewBox.minY}
          width={viewBox.width}
          height={viewBox.height}
          fill="#171717"
        />
        <g stroke="#7c8593" strokeWidth={1.1} opacity={0.62}>
          {graph.edges.map((edge) => {
            const from = positionedById.get(edge.from);
            const to = positionedById.get(edge.to);
            if (!from || !to) {
              return null;
            }
            const dx = to.x - from.x;
            const dy = to.y - from.y;
            const distance = Math.max(1, Math.sqrt(dx * dx + dy * dy));
            const startX = from.x + (dx / distance) * from.radius;
            const startY = from.y + (dy / distance) * from.radius;
            const endX = to.x - (dx / distance) * to.radius;
            const endY = to.y - (dy / distance) * to.radius;
            return (
              <line
                key={`${edge.from}-${edge.to}-${edge.kind}`}
                x1={startX}
                y1={startY}
                x2={endX}
                y2={endY}
              />
            );
          })}
        </g>
        <g>
          {layout.nodes.map((node) => {
            const selected = node.id === selectedNodeId;
            const glowRadius = node.radius + (selected ? 5.5 : 2.4);
            const interactive = node.kind !== "root";
            return (
              <g
                key={node.id}
                onClick={interactive ? () => onSelectNode(node) : undefined}
                style={{ cursor: interactive ? "pointer" : "default" }}
              >
                <title>{node.label}</title>
                <circle
                  cx={node.x}
                  cy={node.y}
                  r={glowRadius}
                  fill={selected ? graphNodeColor(node) : "#ffffff"}
                  opacity={selected ? 0.14 : 0.035}
                />
                <circle
                  cx={node.x}
                  cy={node.y}
                  r={node.radius}
                  fill={graphNodeFill(node)}
                  stroke={selected ? "#ffffff" : graphNodeColor(node)}
                  strokeWidth={selected ? 2.1 : node.kind === "leaf" ? 0.55 : 0.95}
                />
              </g>
            );
          })}
        </g>
      </svg>
    </div>
  );
}

export function MemoryPane({ embedded }: { embedded?: boolean } = {}) {
  const { selectedWorkspace, workspaces } = useWorkspaceDesktop();
  const [activeWorkspaceId, setActiveWorkspaceId] = useState(
    selectedWorkspace?.id?.trim() || "",
  );
  const [viewMode, setViewMode] = useState<MemoryViewMode>("graph");
  const [selectedGraphNodeId, setSelectedGraphNodeId] = useState<string | null>(
    null,
  );
  const [tree, setTree] = useState<MemoryBrowserTreeResponsePayload | null>(
    null,
  );
  const [graph, setGraph] = useState<MemoryBrowserGraphResponsePayload | null>(
    null,
  );
  const [selectedPath, setSelectedPath] = useState<string | null>(null);
  const [selectedFile, setSelectedFile] =
    useState<MemoryBrowserFileResponsePayload | null>(null);
  const [selectedNodeDetail, setSelectedNodeDetail] =
    useState<MemoryBrowserNodeDetailResponsePayload | null>(null);
  const [treeError, setTreeError] = useState("");
  const [graphError, setGraphError] = useState("");
  const [fileError, setFileError] = useState("");
  const [isLoadingTree, setIsLoadingTree] = useState(false);
  const [isLoadingGraph, setIsLoadingGraph] = useState(false);
  const [isLoadingFile, setIsLoadingFile] = useState(false);
  const [expandedPaths, setExpandedPaths] = useState<Set<string>>(
    defaultExpandedPaths,
  );
  const [treeSearchQuery, setTreeSearchQuery] = useState("");
  const workspaceId = activeWorkspaceId.trim();

  useEffect(() => {
    if (selectedWorkspace?.id?.trim()) {
      setActiveWorkspaceId((current) =>
        current.trim() ? current : selectedWorkspace.id.trim(),
      );
      return;
    }
    if (workspaces.length > 0) {
      setActiveWorkspaceId((current) =>
        current.trim() ? current : workspaces[0]?.id?.trim() || "",
      );
    }
  }, [selectedWorkspace?.id, workspaces]);

  const loadFile = useCallback(
    async (targetPath: string) => {
      if (!workspaceId) {
        return;
      }
      setIsLoadingFile(true);
      setFileError("");
      try {
        const response = (await remoteApi.memory.readFile({
          path: targetPath,
        })) as unknown as MemoryBrowserFileResponsePayload;
        setSelectedFile(response);
      } catch (error) {
        setSelectedFile(null);
        setFileError(normalizeErrorMessage(error));
      } finally {
        setIsLoadingFile(false);
      }
    },
    [workspaceId],
  );

  const loadNodeDetail = useCallback(
    async (node: MemoryBrowserGraphNodePayload | null) => {
      if (!workspaceId || !node) {
        setSelectedNodeDetail(null);
        return;
      }
      try {
        const response = (await remoteApi.memory.readNodeDetail({
          nodeId: node.id,
          treeId: node.tree_id ?? undefined,
        })) as unknown as MemoryBrowserNodeDetailResponsePayload;
        setSelectedNodeDetail(response);
      } catch {
        setSelectedNodeDetail(null);
      }
    },
    [workspaceId],
  );

  const loadTree = useCallback(
    async (preferredPath?: string | null) => {
      if (!workspaceId) {
        setTree(null);
        setSelectedPath(null);
        setSelectedFile(null);
        setTreeError("");
        setFileError("");
        setExpandedPaths(defaultExpandedPaths());
        return;
      }
      setIsLoadingTree(true);
      setTreeError("");
      try {
        const response = (await remoteApi.memory.browseTree({
        })) as unknown as MemoryBrowserTreeResponsePayload;
        setTree(response);
        const nextSelectedPath =
          preferredPath && treeHasFile(response.root, preferredPath)
            ? preferredPath
            : firstFilePath(response.root);
        setExpandedPaths((previous) => {
          const next = new Set(previous);
          for (const target of ancestorPaths(nextSelectedPath ?? "")) {
            next.add(target);
          }
          return next;
        });
        setSelectedPath(nextSelectedPath);
        if (nextSelectedPath) {
          await loadFile(nextSelectedPath);
        } else {
          setSelectedFile(null);
          setFileError("");
        }
      } catch (error) {
        setTree(null);
        setSelectedPath(null);
        setSelectedFile(null);
        setTreeError(normalizeErrorMessage(error));
      } finally {
        setIsLoadingTree(false);
      }
    },
    [loadFile, workspaceId],
  );

  const loadGraph = useCallback(
    async (preferredNodeId?: string | null) => {
      if (!workspaceId) {
        setGraph(null);
        setSelectedGraphNodeId(null);
        setGraphError("");
        return;
      }
      setIsLoadingGraph(true);
      setGraphError("");
      try {
        const response = (await remoteApi.memory.browseGraph({
          forest: "workspace",
          maxLayers: MEMORY_GRAPH_MAX_LAYERS,
          maxNodes: MEMORY_GRAPH_MAX_NODES,
        })) as unknown as MemoryBrowserGraphResponsePayload;
        setGraph(response);
        const preferredNode =
          preferredNodeId == null
            ? null
            : response.nodes.find((node) => node.id === preferredNodeId) ?? null;
        const nextSelectedNode =
          preferredNode ??
          response.nodes.find((node) => Boolean(node.path)) ??
          response.nodes.find((node) => node.kind !== "root") ??
          response.nodes.find((node) => node.kind === "root") ??
          response.nodes[0] ??
          null;
        setSelectedGraphNodeId(nextSelectedNode?.id ?? null);
        await loadNodeDetail(nextSelectedNode);
        if (nextSelectedNode?.path) {
          await loadFile(nextSelectedNode.path);
        } else if (!nextSelectedNode) {
          setSelectedFile(null);
          setFileError("");
        }
      } catch (error) {
        setGraph(null);
        setSelectedGraphNodeId(null);
        setGraphError(normalizeErrorMessage(error));
      } finally {
        setIsLoadingGraph(false);
      }
    },
    [loadFile, loadNodeDetail, workspaceId],
  );

  useEffect(() => {
    void loadTree(null);
  }, [loadTree, workspaceId]);

  useEffect(() => {
    void loadGraph(null);
  }, [loadGraph, workspaceId]);

  useEffect(() => {
    setSelectedGraphNodeId(null);
    setSelectedFile(null);
    setSelectedNodeDetail(null);
    setFileError("");
  }, [workspaceId]);

  useEffect(() => {
    if (!graph || !selectedPath || selectedGraphNodeId || selectedNodeDetail) {
      return;
    }
    const matchedGraphNode = findGraphNodeForPath(graph.nodes, selectedPath);
    if (!matchedGraphNode) {
      return;
    }
    setSelectedGraphNodeId(matchedGraphNode.id);
    void loadNodeDetail(matchedGraphNode);
  }, [
    graph,
    loadNodeDetail,
    selectedGraphNodeId,
    selectedNodeDetail,
    selectedPath,
  ]);

  const handleRefresh = useCallback(() => {
    void loadTree(selectedPath);
    void loadGraph(selectedGraphNodeId);
  }, [loadGraph, loadTree, selectedGraphNodeId, selectedPath]);

  const handleSelectFile = useCallback(
    (targetPath: string) => {
      setViewMode("file");
      const matchedGraphNode = findGraphNodeForPath(graph?.nodes, targetPath);
      setSelectedGraphNodeId(matchedGraphNode?.id ?? null);
      if (matchedGraphNode) {
        void loadNodeDetail(matchedGraphNode);
      } else {
        setSelectedNodeDetail(null);
      }
      setSelectedPath(targetPath);
      setExpandedPaths((previous) => {
        const next = new Set(previous);
        for (const target of ancestorPaths(targetPath)) {
          next.add(target);
        }
        return next;
      });
      void loadFile(targetPath);
    },
    [graph, loadFile, loadNodeDetail],
  );

  const handleToggleDirectory = useCallback((targetPath: string) => {
    setExpandedPaths((previous) => {
      const next = new Set(previous);
      if (next.has(targetPath)) {
        next.delete(targetPath);
      } else {
        next.add(targetPath);
      }
      return next;
    });
  }, []);

  const handleSelectGraphNode = useCallback(
    (node: MemoryBrowserGraphNodePayload) => {
      setViewMode("file");
      setSelectedGraphNodeId(node.id);
      void loadNodeDetail(node);
      if (node.path) {
        setSelectedPath(node.path);
        setExpandedPaths((previous) => {
          const next = new Set(previous);
          for (const target of ancestorPaths(node.path ?? "")) {
            next.add(target);
          }
          return next;
        });
        void loadFile(node.path);
      } else {
        setSelectedPath(null);
        setSelectedFile(null);
        setFileError("");
      }
    },
    [loadFile, loadNodeDetail],
  );

  const handleSelectGraphNodeById = useCallback(
    (nodeId: string | null) => {
      if (!nodeId) {
        return;
      }
      const matchedNode =
        graph?.nodes.find((candidate) => candidate.id === nodeId) ?? null;
      if (matchedNode) {
        handleSelectGraphNode(matchedNode);
        return;
      }
      void loadGraph(nodeId);
    },
    [graph, handleSelectGraphNode, loadGraph],
  );

  const visibleRoot = tree?.root ?? null;

  const selectedGraphNode = useMemo(
    () =>
      graph?.nodes.find((node) => node.id === selectedGraphNodeId) ??
      null,
    [graph, selectedGraphNodeId],
  );
  const relatedTargetResolution = useMemo(
    () =>
      buildRelatedTargetResolution({
        graphNodes: graph?.nodes,
        outgoingRelations: selectedNodeDetail?.outgoing_relations,
      }),
    [graph, selectedNodeDetail],
  );
  const selectedFileRelatedInfo = useMemo(
    () => selectedFile ? parseMemoryRelatedSections(selectedFile.content) : { entities: [], relations: [] },
    [selectedFile],
  );
  const selectedFileRelatedEntities = useMemo(
    () =>
      resolveParsedRelatedEntities(
        relatedTargetResolution,
        selectedFileRelatedInfo.entities,
      ),
    [relatedTargetResolution, selectedFileRelatedInfo.entities],
  );
  const selectedFileRelatedRelations = useMemo(
    () =>
      resolveParsedRelatedRelations(
        relatedTargetResolution,
        selectedFileRelatedInfo.relations,
      ),
    [relatedTargetResolution, selectedFileRelatedInfo.relations],
  );

  if (workspaces.length === 0) {
    return (
      <SettingsCard>
        <div className="p-4 text-sm text-muted-foreground">
          No workspace is selected.
        </div>
      </SettingsCard>
    );
  }

  const filesCount = visibleRoot ? countMemoryFiles(visibleRoot) : 0;
  const trimmedQuery = treeSearchQuery.trim().toLowerCase();
  const displayRoot =
    visibleRoot && trimmedQuery
      ? filterMemoryTreeByQuery(visibleRoot, trimmedQuery)
      : visibleRoot;

  const graphNodeCount = graph?.nodes.length ?? 0;
  const graphEdgeCount = graph?.edges.length ?? 0;
  const refreshing = viewMode === "graph" ? isLoadingGraph : isLoadingTree;

  const hasNodeDetails = Boolean(
    selectedNodeDetail &&
      (selectedNodeDetail.evidence_refs.length > 0 ||
        selectedNodeDetail.outgoing_relations.length > 0 ||
        selectedNodeDetail.incoming_relations.length > 0),
  );
  const hasFileDetails = Boolean(
    selectedFile &&
      (selectedFileRelatedEntities.length > 0 ||
        selectedFileRelatedRelations.length > 0),
  );
  const showInspector = Boolean(
    selectedGraphNode || hasNodeDetails || hasFileDetails,
  );
  const inspectorTitle =
    selectedGraphNode?.label ??
    (selectedFile?.path ? selectedFile.path.split("/").pop() : null) ??
    "Inspector";

  return (
    <SettingsCard className={embedded ? "" : "overflow-hidden"}>
      <div
        className={cn(
          "grid h-[560px]",
          // Inspector metadata moved into the file view header strip
          // below, so the layout is always two columns regardless of
          // selection. Frees real estate for the file content itself
          // — the third column was squeezing the middle into a
          // ~150px ribbon and wrapping content vertically.
          "grid-cols-[220px_minmax(0,1fr)] xl:grid-cols-[240px_minmax(0,1fr)]",
        )}
      >
        {/* Tree */}
        <div className="flex min-h-0 flex-col border-r border-border bg-fg-2">
          <div className="flex shrink-0 items-baseline justify-between gap-2 border-b border-border px-3 py-2">
            <div className="flex items-baseline gap-1.5">
              <span className="text-xs font-medium text-foreground">Files</span>
              <span className="text-[11px] tabular-nums text-muted-foreground">
                {filesCount}
              </span>
            </div>
          </div>
          {filesCount > 0 ? (
            <div className="shrink-0 border-b border-border px-2 py-1.5">
              <div className="relative">
                <Search className="pointer-events-none absolute left-2 top-1/2 size-3 -translate-y-1/2 text-muted-foreground" />
                <input
                  type="text"
                  value={treeSearchQuery}
                  onChange={(e) => setTreeSearchQuery(e.target.value)}
                  placeholder="Search files"
                  aria-label="Search memory files"
                  className="embedded-input h-7 w-full rounded-md border border-border bg-background pl-7 pr-2 text-xs text-foreground outline-none placeholder:text-muted-foreground focus:border-ring"
                />
              </div>
            </div>
          ) : null}
          <div className="min-h-0 flex-1 overflow-auto p-2">
            {treeError ? (
              <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3 text-xs text-destructive">
                {treeError}
              </div>
            ) : isLoadingTree && !visibleRoot ? (
              <div className="flex items-center gap-2 p-3 text-xs text-muted-foreground">
                <Loader2 className="size-3.5 animate-spin" />
                Loading…
              </div>
            ) : displayRoot ? (
              <MemoryTreeEntry
                node={displayRoot}
                depth={0}
                expandedPaths={expandedPaths}
                selectedPath={selectedPath}
                onToggleDirectory={handleToggleDirectory}
                onSelectFile={handleSelectFile}
              />
            ) : visibleRoot && trimmedQuery ? (
              <div className="p-3 text-xs text-muted-foreground">
                No files match "{treeSearchQuery.trim()}".
              </div>
            ) : (
              <div className="p-3 text-xs text-muted-foreground">
                No memory files yet.
              </div>
            )}
          </div>
        </div>

        {/* Content */}
        <div className="flex min-h-0 flex-col">
          <div className="flex shrink-0 items-center justify-between gap-3 border-b border-border px-3 py-2">
            <div className="flex items-center gap-0.5 rounded-md border border-border bg-background p-0.5">
              {(
                [
                  { id: "graph", label: "Graph" },
                  { id: "file", label: "File" },
                ] as const
              ).map((option) => (
                <Button
                  key={option.id}
                  type="button"
                  size="xs"
                  variant={viewMode === option.id ? "secondary" : "ghost"}
                  onClick={() => setViewMode(option.id)}
                  className="h-6 px-2 text-[11px]"
                >
                  {option.label}
                </Button>
              ))}
            </div>
            <div className="flex min-w-0 items-center gap-2">
              {viewMode === "graph" && graph ? (
                <span className="text-[11px] tabular-nums text-muted-foreground">
                  {graphNodeCount} nodes · {graphEdgeCount} edges
                </span>
              ) : null}
              {viewMode === "file" && selectedFile ? (
                <>
                  <span
                    className="min-w-0 truncate font-mono text-[10.5px] text-muted-foreground"
                    title={selectedFile.path}
                  >
                    {selectedFile.path}
                  </span>
                  <span className="shrink-0 text-[11px] tabular-nums text-muted-foreground">
                    {formatBytes(selectedFile.size_bytes)}
                  </span>
                </>
              ) : null}
              <DropdownMenu>
                <DropdownMenuTrigger
                  render={
                    <Button
                      variant="ghost"
                      size="icon-sm"
                      aria-label="More actions"
                    />
                  }
                >
                  <MoreHorizontal className="size-3.5" />
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" sideOffset={6} className="w-40">
                  <DropdownMenuItem
                    onClick={handleRefresh}
                    disabled={refreshing}
                  >
                    <RefreshCw
                      className={cn("size-3.5", refreshing && "animate-spin")}
                    />
                    Refresh
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
          </div>

          <div className="min-h-0 flex-1 overflow-auto">
            {/* Inspector lives above File view only — the graph already
                surfaces node identity through selection + the side
                legend, and stacking it above the SVG canvas pushes the
                graph off-screen. */}
            {viewMode === "file" && showInspector ? (
              <div className="border-b border-border bg-fg-2 px-4 py-3">
                <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                  <div
                    className="min-w-0 flex-1 truncate text-sm font-medium text-foreground"
                    title={inspectorTitle}
                  >
                    {inspectorTitle}
                  </div>
                  {selectedGraphNode ? (
                    <div className="flex flex-wrap items-center gap-1">
                      <Badge variant="outline" className="bg-background">
                        {selectedGraphNode.kind}
                      </Badge>
                      {selectedGraphNode.status ? (
                        <Badge variant="outline" className="bg-background">
                          {selectedGraphNode.status}
                        </Badge>
                      ) : null}
                      {selectedGraphNode.level != null ? (
                        <Badge variant="outline" className="bg-background">
                          L{selectedGraphNode.level}
                        </Badge>
                      ) : null}
                      {selectedGraphNode.child_count != null ? (
                        <Badge variant="outline" className="bg-background">
                          {selectedGraphNode.child_count} children
                        </Badge>
                      ) : null}
                    </div>
                  ) : null}
                </div>
                {selectedGraphNode?.subtitle ? (
                  <p className="mt-1 line-clamp-3 text-[11px] text-muted-foreground">
                    {selectedGraphNode.subtitle}
                  </p>
                ) : null}
                {selectedGraphNode?.tree_id ? (
                  <p
                    className="mt-1 truncate text-[10.5px] text-muted-foreground"
                    title={selectedGraphNode.tree_id}
                  >
                    Tree:{" "}
                    <span className="font-mono">
                      {selectedGraphNode.tree_id}
                    </span>
                  </p>
                ) : null}
                {selectedGraphNode?.path && !selectedFile ? (
                  <p
                    className="mt-1 break-all font-mono text-[10.5px] text-muted-foreground"
                    title={selectedGraphNode.path}
                  >
                    {selectedGraphNode.path}
                  </p>
                ) : null}
                {(selectedNodeDetail && hasNodeDetails) || hasFileDetails ? (
                  <div className="mt-3 grid gap-3 lg:grid-cols-2">
                    {selectedNodeDetail && hasNodeDetails ? (
                      <>
                        {selectedNodeDetail.evidence_refs.length > 0 ? (
                          <div className="grid gap-1.5">
                            <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                              Evidence
                            </div>
                            <div className="grid gap-1.5">
                              {selectedNodeDetail.evidence_refs.map((ref) => (
                                <div
                                  key={ref.ref_id}
                                  className="rounded-md border border-border bg-background p-2 text-[11px] text-muted-foreground"
                                >
                                  <div className="font-medium text-foreground">
                                    {ref.provider && ref.account_namespace
                                      ? `${ref.provider} ${ref.account_namespace}`
                                      : ref.source_type ?? ref.ref_id}
                                  </div>
                                  <div className="text-[10.5px]">
                                    {[
                                      ref.external_object_type,
                                      ref.external_object_id,
                                      ref.source_turn_input_id,
                                    ]
                                      .filter(Boolean)
                                      .join(" • ")}
                                  </div>
                                </div>
                              ))}
                            </div>
                          </div>
                        ) : null}

                        {selectedNodeDetail.outgoing_relations.length > 0 ? (
                          <div className="grid gap-1.5">
                            <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                              Connects to
                            </div>
                            <div className="grid gap-0.5">
                              {selectedNodeDetail.outgoing_relations.map(
                                (relation) => {
                                  const relationContent = (
                                    <>
                                      <div className="flex items-center gap-1.5">
                                        <span className="text-[11px] font-medium text-foreground">
                                          {relation.relation_type.replaceAll(
                                            "_",
                                            " ",
                                          )}
                                        </span>
                                        <Badge
                                          variant="outline"
                                          className={relationResolutionBadgeClass(
                                            relation.target_resolution_kind,
                                          )}
                                        >
                                          {relationResolutionLabel(
                                            relation.target_resolution_kind,
                                          )}
                                        </Badge>
                                      </div>
                                      <div className="truncate text-[11px]">
                                        {relation.target_label ??
                                          relation.target_entity_key ??
                                          relation.target_node_id}
                                      </div>
                                    </>
                                  );
                                  if (
                                    !isNavigableMemoryRelationTarget(
                                      relation.target_resolution_kind,
                                    )
                                  ) {
                                    return (
                                      <div
                                        key={`out:${relation.relation_type}:${relation.target_node_id}`}
                                        className="grid gap-0.5 rounded px-1.5 py-1 text-[11px] text-muted-foreground"
                                      >
                                        {relationContent}
                                      </div>
                                    );
                                  }
                                  return (
                                    <button
                                      key={`out:${relation.relation_type}:${relation.target_node_id}`}
                                      type="button"
                                      onClick={() =>
                                        handleSelectGraphNodeById(
                                          relation.target_node_id,
                                        )
                                      }
                                      className="grid gap-0.5 rounded px-1.5 py-1 text-left text-[11px] text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                                    >
                                      {relationContent}
                                    </button>
                                  );
                                },
                              )}
                            </div>
                          </div>
                        ) : null}

                        {selectedNodeDetail.incoming_relations.length > 0 ? (
                          <div className="grid gap-1.5">
                            <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                              Referenced by
                            </div>
                            <div className="grid gap-0.5">
                              {selectedNodeDetail.incoming_relations.map(
                                (relation) => (
                                  <button
                                    key={`in:${relation.relation_type}:${relation.source_node_id}`}
                                    type="button"
                                    onClick={() =>
                                      handleSelectGraphNodeById(
                                        relation.source_node_id,
                                      )
                                    }
                                    className="grid gap-0.5 rounded px-1.5 py-1 text-left text-[11px] text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                                  >
                                    <div className="flex items-center gap-1.5">
                                      <span className="truncate font-medium text-foreground">
                                        {relation.source_label ??
                                          relation.source_node_id}
                                      </span>
                                      <Badge
                                        variant="outline"
                                        className={relationResolutionBadgeClass(
                                          relation.target_resolution_kind,
                                        )}
                                      >
                                        {relationResolutionLabel(
                                          relation.target_resolution_kind,
                                        )}
                                      </Badge>
                                    </div>
                                    <div className="text-[11px]">
                                      via{" "}
                                      {relation.relation_type.replaceAll(
                                        "_",
                                        " ",
                                      )}
                                    </div>
                                  </button>
                                ),
                              )}
                            </div>
                          </div>
                        ) : null}
                      </>
                    ) : null}

                    {hasFileDetails ? (
                      <>
                        {selectedFileRelatedEntities.length > 0 ? (
                          <div className="grid gap-1.5">
                            <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                              Related entities
                            </div>
                            <div className="flex flex-wrap gap-1.5">
                              {selectedFileRelatedEntities.map((entity) => {
                                const badge = entity.targetResolutionKind ? (
                                  <Badge
                                    variant="outline"
                                    className={relationResolutionBadgeClass(
                                      entity.targetResolutionKind,
                                    )}
                                  >
                                    {relationResolutionLabel(
                                      entity.targetResolutionKind,
                                    )}
                                  </Badge>
                                ) : null;
                                if (!entity.navigable || !entity.nodeId) {
                                  return (
                                    <div
                                      key={entity.stableKey}
                                      className="flex items-center gap-1"
                                    >
                                      <Badge
                                        variant="outline"
                                        className="bg-background"
                                      >
                                        {entity.label}
                                      </Badge>
                                      {badge}
                                    </div>
                                  );
                                }
                                return (
                                  <button
                                    key={entity.stableKey}
                                    type="button"
                                    onClick={() =>
                                      handleSelectGraphNodeById(entity.nodeId)
                                    }
                                    className="flex items-center gap-1 rounded transition-opacity hover:opacity-80"
                                  >
                                    <Badge
                                      variant="outline"
                                      className="bg-background"
                                    >
                                      {entity.label}
                                    </Badge>
                                    {badge}
                                  </button>
                                );
                              })}
                            </div>
                          </div>
                        ) : null}

                        {selectedFileRelatedRelations.length > 0 ? (
                          <div className="grid gap-1.5">
                            <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                              Relations
                            </div>
                            <div className="grid gap-0.5">
                              {selectedFileRelatedRelations.map((relation) => {
                                const content = (
                                  <div className="flex items-center gap-1.5">
                                    <span className="text-[11px] font-medium text-foreground">
                                      {relation.relationType.replaceAll(
                                        "_",
                                        " ",
                                      )}
                                    </span>
                                    <span className="truncate text-[11px]">
                                      {relation.displayLabel}
                                    </span>
                                    {relation.targetResolutionKind ? (
                                      <Badge
                                        variant="outline"
                                        className={relationResolutionBadgeClass(
                                          relation.targetResolutionKind,
                                        )}
                                      >
                                        {relationResolutionLabel(
                                          relation.targetResolutionKind,
                                        )}
                                      </Badge>
                                    ) : null}
                                  </div>
                                );
                                if (!relation.navigable || !relation.nodeId) {
                                  return (
                                    <div
                                      key={relation.stableKey}
                                      className="rounded px-1.5 py-1 text-[11px] text-muted-foreground"
                                    >
                                      {content}
                                    </div>
                                  );
                                }
                                return (
                                  <button
                                    key={relation.stableKey}
                                    type="button"
                                    onClick={() =>
                                      handleSelectGraphNodeById(relation.nodeId)
                                    }
                                    className="rounded px-1.5 py-1 text-left text-[11px] text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                                  >
                                    {content}
                                  </button>
                                );
                              })}
                            </div>
                          </div>
                        ) : null}
                      </>
                    ) : null}
                  </div>
                ) : null}
              </div>
            ) : null}
            {viewMode === "graph" ? (
              graphError ? (
                <div className="m-4 rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">
                  {graphError}
                </div>
              ) : isLoadingGraph && !graph ? (
                <div className="flex items-center gap-2 p-4 text-xs text-muted-foreground">
                  <Loader2 className="size-3.5 animate-spin" />
                  Loading graph…
                </div>
              ) : graph ? (
                <MemoryGraphCanvas
                  graph={graph}
                  selectedNodeId={selectedGraphNodeId}
                  onSelectNode={handleSelectGraphNode}
                />
              ) : (
                <div className="grid h-full place-items-center p-6">
                  <EmptyState
                    icon={BrainCircuit}
                    title="No graph yet"
                    description="The memory graph will appear once entities have been extracted from this workspace's notes."
                    size="md"
                  />
                </div>
              )
            ) : fileError ? (
              <div className="m-4 rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">
                {fileError}
              </div>
            ) : isLoadingFile ? (
              <div className="flex items-center gap-2 p-4 text-xs text-muted-foreground">
                <Loader2 className="size-3.5 animate-spin" />
                Loading file…
              </div>
            ) : selectedFile ? (
              <pre className="m-4 overflow-x-auto whitespace-pre-wrap rounded-md border border-border bg-fg-2 p-3 text-xs leading-6 text-foreground">
                {selectedFile.content}
              </pre>
            ) : selectedGraphNode ? (
              <div className="p-4 text-xs text-muted-foreground">
                This node does not have a backing markdown file.
              </div>
            ) : (
              <div className="grid h-full place-items-center p-6">
                <EmptyState
                  icon={FileText}
                  title="Nothing selected"
                  description="Pick a file from the left, or click a node in the graph."
                  size="md"
                />
              </div>
            )}
          </div>
        </div>

      </div>
    </SettingsCard>
  );
}
