import { Check, Loader2, Pencil, X } from "@/components/ui/icons";
import {
  type ChangeEvent,
  type KeyboardEvent,
  useEffect,
    ...
    <div className="grid gap-1.5">
      ...
      {editing ? (
        ...
        >
          ...
          <span
            aria-hidden="true"
            className="pointer-events-none absolute right-1.5 top-1.5 inline-flex size-5 items-center justify-center rounded text-muted-foreground opacity-50 transition-opacity group-hover/inst-display:opacity-100"
          >
      ...
      <PopoverTrigger
        render={
          ...
          >
            ...
            <Pencil
              aria-hidden="true"
              className="size-3 text-muted-foreground/50 transition-colors group-hover/sched-display:text-muted-foreground"
            />
          </button>
        }
      />
          ...
          control={
            ...
            >
              <SelectTrigger className="h-8 text-xs">
                <span>
                  {SCHEDULE_PRESET_OPTIONS.find((o) => o.kind === preset.kind)
                    ?.label ?? preset.kind}
                </span>
              </SelectTrigger>
...
/**
 * Controlled cron builder body (preset picker + parametric controls + live
 * preview), without the popover's Save/Cancel footer. For use inside create
 * forms (e.g. the manual "New automation" dialog). Reports the current cron and
 * its validity up on every change.
 */
export function SchedulePicker({
  ...
  cron: string;
  onChange: (result: { cron: string; valid: boolean }) => void;
}) {
  const [preset, setPreset] = useState<SchedulePreset>(() => cronToPreset(cron));
  const [customDraft, setCustomDraft] = useState<string>(
    () => cronToPreset(cron).customCron ?? cron,
  );
  const candidateCron =
    preset.kind === "custom" ? customDraft : presetToCron(preset);
  const validation = useMemo(() => validateCron(candidateCron), [candidateCron]);
  // Notify the parent without requiring a stable onChange identity.
  const onChangeRef = useRef(onChange);
  onChangeRef.current = onChange;
  useEffect(() => {
    onChangeRef.current({ cron: candidateCron.trim(), valid: validation.valid });
  }, [candidateCron, validation.valid]);
  ...
  return (
    <div className="grid gap-3">
      <PresetField
        ...
        control={
          <Select
            ...
            onValueChange={(next) => {
              if (next == null) return;
              setPreset(initialPresetFor(next as SchedulePresetKind, preset));
            }}
          >
            <SelectTrigger className="h-8 text-xs">
              <span>
      ...
      <PresetParametricControls
        ...
        onCustomDraftChange={setCustomDraft}
      />
      {/* The Repeat + parametric controls already read as plain English, so the
          raw-cron preview is redundant noise; only surface a message when a
          custom cron is invalid. */}
      {!validation.valid ? (
...
function NumberSelect({
  ...
  return (
    ...
    >
      ...
      {/* Non-aligned: open as a normal dropdown from the top. The default
          aligned mode anchors the selected item to the trigger, which floats a
          scroll-up arrow over the first items so clicks there scroll instead of
          selecting — bad for the long hour/minute lists. */}
      <SelectContent
        alignItemWithTrigger={false}
        className={cn("bg-popover", options.length > 12 && "max-h-60")}
      >
        {options.map((opt) => (
          <SelectItem
            key={opt}
...
function TimePicker({ hour, minute, onChange }: TimePickerProps) {
  // w-fit + flex so the hour / minute stay adjacent regardless of how wide the
  // containing field is (a full-width dialog would otherwise stretch them
  // apart with the previous 1fr_auto_1fr grid).
  return (
