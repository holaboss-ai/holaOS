      ...
      <div className="min-h-0 flex-1 overflow-auto">
        <table className="w-max min-w-full border-separate border-spacing-0 text-sm text-foreground">
          <thead className="sticky top-0 z-2">
            <tr>
              <th className="sticky left-0 z-3 w-11 border-b border-r border-border bg-muted px-0 py-0 text-center text-xs font-normal uppercase text-muted-foreground">
                <div className="flex h-8 items-center justify-center">#</div>
              </th>
              {activeSheet.columns.map((column, columnIndex) => (
                <th
                  key={`${column}-${columnIndex}`}
                  className="min-w-[172px] border-b border-r border-border bg-muted px-0 py-0 text-left text-xs font-medium text-foreground last:border-r-0"
                >
              ...
              activeSheet.rows.map((row, rowIndex) => {
                return (
                  <tr key={`row-${rowIndex}`} className="group/row">
                    <td
                      className="sticky left-0 z-[1] w-11 border-b border-r border-border bg-background px-0 py-0 text-center align-middle text-xs text-muted-foreground transition-colors group-hover/row:bg-accent/25 group-hover/row:text-muted-foreground"
                    >
