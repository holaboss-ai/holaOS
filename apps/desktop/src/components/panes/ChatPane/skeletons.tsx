import { Check, ChevronRight, CircleAlert, RotateCw } from "@/components/ui/icons";
...
function AssistantTurnSkeleton({ widths }: { widths: string[] }) {
  return (
...
export function HistoryRestoreSkeleton() {
  return (
    <div
      ...
      className="absolute inset-0 z-30 flex flex-col overflow-hidden bg-card"
      role="status"
    >
      <div className="min-h-0 flex-1 overflow-hidden px-6 pt-8">
        <div className="mx-auto flex max-w-3xl animate-pulse flex-col gap-8">
          <AssistantTurnSkeleton widths={["100%", "88%", "54%"]} />
          <div className="flex justify-end">
            <div className="w-[55%] space-y-2 rounded-2xl rounded-br-md bg-muted px-4 py-3">
              <div className="h-3.5 w-full rounded bg-fg-8" />
              <div className="h-3.5 w-2/3 rounded bg-fg-8" />
            </div>
          </div>
          <AssistantTurnSkeleton widths={["100%", "94%", "72%"]} />
        </div>
      </div>

      <div className="shrink-0 px-6 pt-2 pb-6">
        <div className="mx-auto h-[88px] max-w-3xl animate-pulse rounded-2xl border border-border bg-muted" />
      </div>
    </div>
  );
}
