...
export function LiveStatusEllipsis() {
  return (
    ...
    >
      <DotmSquare3 dotSize={1.5} size={14} />
    </span>
  );
}
...
export function LiveStatusLine({
  ...
  return (
    <div
      ...
      key={normalizedLabel}
      className={cn(
        "flex w-fit items-center gap-2 text-sm leading-none text-muted-foreground animate-in fade-in-0 slide-in-from-bottom-0.5 duration-snappy ease-out-expo",
        className,
      )}
    >
      <LiveStatusEllipsis />
  ...
  return (
    <div>
      ...
      >
        <span className="grid size-4 shrink-0 place-items-center">
          {step.status === "completed" ? (
            <Check
              className="size-3 text-success motion-safe:animate-in motion-safe:fade-in-0 motion-safe:zoom-in-75 motion-safe:duration-snappy motion-safe:ease-out-expo"
        ...
        {/* Title + first-detail share one truncating line so a long step
            title (e.g. a fully-qualified subagent name) can't push the
            row past the chat-pane width. */}
        <span className="block min-w-0 flex-auto truncate text-muted-foreground">
          <span className="font-medium text-foreground">{step.title}</span>
          {hasDetails ? <> {step.details[0]}</> : null}
        </span>
        {expandable ? (
          <ChevronDown
      ...
      {expanded && expandable && !suppressLegacyDetails(step) ? (
        <div className="ml-6 mb-1 mt-1 overflow-hidden rounded-lg bg-fg-6 px-3.5 py-2.5 text-xs leading-5 text-muted-foreground">
