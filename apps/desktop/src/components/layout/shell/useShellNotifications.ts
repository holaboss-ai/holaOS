...
function notificationToolKind(
  notification: RuntimeNotificationRecordPayload,
): string | null {
  return notificationMetadataString(notification, "tool_kind");
}

function isSystemCronjobNotification(
  notification: RuntimeNotificationRecordPayload,
): boolean {
  const sourceType = notification.source_type.trim().toLowerCase();
  if (
    sourceType === "cronjob" &&
  ...
  return (
    sourceType === "workflow" &&
    notificationToolKind(notification) === "system_notification"
  );
}
...
export function useShellNotifications(): UseShellNotificationsResult {
      ...
      if (windowAway) {
        ...
        try {
          ...
          if (shown) {
            ...
            try {
              await remoteApi.notifications.update({
  ...
  const refresh = useCallback(
    async (signal: { cancelled: boolean }) => {
      ...
      try {
        const response = await remoteApi.notifications.list({
