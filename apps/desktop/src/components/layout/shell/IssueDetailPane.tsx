...
export function IssueDetailPane({
  ...
  const handleBackToBoard = useCallback(() => {
    ...
    const currentTabId = makeIssueDetailTabId(workspaceId, issueId);
    setInternalTabs((prev) => {
      const idx = prev.findIndex((tab) => tab.id === currentTabId);
      if (idx < 0) {
        return prev;
      }
      const next = prev.filter((tab) => tab.id !== currentTabId);
  ...
  const handleSaveDetails = useCallback(async () => {
    ...
    if (issue.status === "blocked" && !normalizedBlockerReason) {
      setMutationError("Waiting issues need a blocker reason.");
      return;
    }
  ...
  // Agent-spawned background tasks get a task-shaped detail (goal → progress →
  // result) instead of the issue-tracker chrome. User-created issues are
  // untouched.
  const isAgentTask = (issue.created_by || "").trim() === "workspace_agent";
  const isTaskRunning = Boolean(issue.active_subagent_id);
  const isTaskBlocked = issue.status === "blocked";
  const isTaskDone = issue.status === "done";
  // Result = the agent's final answer, rendered in ONE place so it never
  // "jumps": while running it's the live stream; once settled it's the last
  // committed assistant message. Progress (the full trace) stays a
  // collapsed-by-default disclosure, so the answer only visibly streams here.
  const outputTextOfSegments = (
    segments: ChatAssistantSegment[] | undefined,
  ): string =>
    (segments ?? [])
      .filter((segment) => segment.kind === "output")
      .map((segment) => (segment.kind === "output" ? segment.text : ""))
      .join("\n\n")
      .trim();
  const lastCommittedAssistant = [...messages]
    .reverse()
    .find((message) => message.role === "assistant");
  const settledResultText = lastCommittedAssistant
    ? outputTextOfSegments(lastCommittedAssistant.segments) ||
      lastCommittedAssistant.text.trim()
    : "";
  const liveResultText =
    outputTextOfSegments(renderedLiveAssistantSegments) ||
    (liveAssistantText || "").trim();
  const taskResultText = isTaskRunning ? liveResultText : settledResultText;
  const taskNowActivity =
    (liveAgentStatus || "").trim() || (isResponding ? "Working…" : "");
  const taskStatusLabel = isTaskRunning
    ? "Working"
    : isTaskBlocked
      ? "Needs you"
      : isTaskDone
  ...
  const taskStatusClass = isTaskRunning
    ...
    <div className="flex h-full min-h-0 flex-col bg-background">
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        <div
          className={
            isAgentTask
              ? "mx-auto w-full max-w-4xl px-8 py-6"
              : "mx-auto grid w-full gap-8 px-8 py-6 xl:grid-cols-[minmax(0,820px)_240px]"
          }
        >
          <article className="min-w-0 space-y-6">
            ...
            {isAgentTask && !isEditingDetails ? (
              <div className="-mt-2 space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  <span
                    className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[12px] font-medium ${taskStatusClass}`}
                  >
                    {isTaskRunning ? (
                  ...
                  {isTaskRunning ? (
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      className="h-7 gap-1.5 border-amber-500/30 bg-amber-500/10 text-[12px] text-amber-800 hover:bg-amber-500/15 hover:text-amber-800 dark:text-amber-100 dark:hover:text-amber-100"
                ...
                <div className="text-[12px] text-foreground/38">
            ...
            {isAgentTask && !isEditingDetails ? (
              <section className="space-y-2.5">
                ...
                {taskResultText ? (
                  <div className="rounded-xl border border-border bg-card px-4 py-3.5 text-[14px] leading-7 text-foreground/85">
                    <SimpleMarkdown
                      className="chat-markdown max-w-full text-foreground/85"
                      onLinkClick={(url) => {
                  ...
                  <div className="flex items-center gap-2.5 rounded-xl border border-dashed border-border bg-background/40 px-4 py-4 text-[13px] text-foreground/45">
                    {isTaskRunning ? (
                      <Loader2 className="size-4 shrink-0 animate-spin text-foreground/35" />
                    ) : null}
                    {isTaskRunning
                      ? "The result will appear here when the task finishes."
                      : "No result produced yet."}
                  </div>
                )}
              </section>
            ) : null}
            ...
            {isEditingDetails ? (
              <Textarea
                ...
                className="min-h-[120px] resize-y bg-background/45 text-[14px] leading-7"
              />
            ) : issue.description && !isAgentTask ? (
              <div className="whitespace-pre-wrap text-[14px] leading-7 text-foreground/78">
                {issue.description}
              </div>
            ) : null}

            {issue.blocker_reason && !isEditingDetails ? (
              <div className="flex items-start gap-3 rounded-md border-l-2 border-amber-500/45 bg-amber-500/[0.05] py-2.5 pl-3 pr-2.5">
            ...
            <div className="space-y-4">
              {isAgentTask ? (
                <button
                  ...
                  className="flex w-full items-center gap-2 text-[11px] font-semibold uppercase tracking-wide text-foreground/45 transition-colors hover:text-foreground/70"
                >
                  <ChevronRight
                    className={`size-3.5 transition-transform${progressOpen ? " rotate-90" : ""}`}
                  />
                  <span>Progress</span>
                  {isTaskRunning && taskNowActivity ? (
                    <span className="flex min-w-0 items-center gap-1.5 font-normal normal-case tracking-normal text-foreground/60">
                      <Loader2 className="size-3 shrink-0 animate-spin text-primary" />
                      <span className="truncate">{taskNowActivity}</span>
                    </span>
                  ) : null}
              ...
              {!isAgentTask || progressOpen ? (
                <>
                  ...
                  !showLiveAssistantTurn ? (
                    ...
                    <ConversationTurns
                      messages={messages}
                      assistantLabel="Background agent"
                      assistantMode="issue"
            ...
            {isAgentTask && issue.description ? (
              <section className="border-t border-border/55 pt-4">
                <button
                  type="button"
                  onClick={() => setInstructionsOpen((value) => !value)}
                  className="flex items-center gap-1.5 text-[12px] font-medium text-foreground/45 transition-colors hover:text-foreground/70"
                >
                  <ChevronRight
                ...
                {instructionsOpen ? (
                  <div className="mt-3 whitespace-pre-wrap text-[13px] leading-7 text-foreground/70">
                    {issue.description}
                  </div>
                ) : null}
              </section>
          ...
          <aside className="space-y-5 xl:sticky xl:top-0 xl:self-start xl:border-l xl:border-border/55 xl:pl-6">
            ...
            {issue.active_subagent_id ? (
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                className="h-7 w-full justify-center gap-1.5 border-amber-500/30 bg-amber-500/10 text-[11px] text-amber-800 hover:bg-amber-500/15 hover:text-amber-800 dark:text-amber-100 dark:hover:text-amber-100"
                onClick={() => void handleResumeIssueRun()}
                disabled={isMutationPending}
              >
                <Play className="size-3" />
