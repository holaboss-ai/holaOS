...
import { Loader2, Paperclip, X } from "@/components/ui/icons";
import {
  type ChangeEvent,
  type FormEvent,
  type KeyboardEvent,
  ...
  const handleSubmit = useCallback(
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      if (status === "blocked" && !blockerReason.trim()) {
        setErrorMessage("Waiting issues need a blocker reason.");
        return;
      }
    ...
    <DialogPrimitive.Root open={open} onOpenChange={handleOpenChange}>
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Popup
          className="fixed top-[12%] left-1/2 z-[100] w-[min(760px,calc(100vw-32px))] -translate-x-1/2 overflow-hidden rounded-xl border border-border bg-popover/95 shadow-2xl outline-none backdrop-blur-2xl ease-emphasized data-open:duration-snappy data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:duration-tap data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95"
        >
          <form
              ...
              <div className="flex items-center gap-3">
                <span className="text-xs text-foreground/45">
                  <kbd className="rounded border border-border bg-foreground/[0.04] px-1 py-0.5 font-sans text-[10px] text-foreground/65">
                    {modifierKeyLabel()} {shortcutKeyLabel("↵")}
                  </kbd>{" "}
                  to create
                </span>
                <Button
