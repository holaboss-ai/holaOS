...
test("notification toast stack anchors itself to the top right corner", async () => {
  ...
  assert.match(
    source,
    /pointer-events-none fixed right-4 top-4 z-\[90\] flex w-\[min\(\d+px,calc\(100vw-2rem\)\)\] flex-col gap-3 sm:right-6 sm:top-6/,
  );
...
test("notification toast stack collapses by default and expands on hover or focus", async () => {
  ...
  assert.match(source, /return <CircleCheck className="size-3\.5" \/>;/);
  ...
  assert.match(source, /transition-\[margin,transform,opacity,max-height\]/);
  assert.match(source, /\.\.\.toastCardStyle\(index, notifications\.length, isExpanded\),/);
  assert.match(source, /style=\{cardStyle\}/);
  ...
  assert.match(source, /isCollapsedBackgroundToast\s*\?\s*"pointer-events-none shadow-\w+"/);
