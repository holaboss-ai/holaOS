...
test("before-quit waits for embedded runtime cleanup before allowing Electron to exit", async () => {
  ...
  assert.match(source, /let appUpdateInstallInProgress = false;/);
  assert.match(
    source,
  ...
  assert.match(
    source,
    /app\.on\("before-quit", \(event\) => \{[\s\S]*if \(appQuitCleanupFinished \|\| appUpdateInstallInProgress\) \{\s*return;\s*\}[\s\S]*event\.preventDefault\(\);[\s\S]*void ensureAppQuitCleanup\(\)\.finally\(\(\) => \{\s*app\.quit\(\);\s*\}\);[\s\S]*\}\);/,
  );
});
