...
test("intel macOS desktop workflow publishes a notarized x64 DMG and updater assets", async () => {
  ...
  assert.doesNotMatch(workflowSource, /--prepackaged "\$\{app_path\}" \\\n\s+--mac dmg zip \\\n\s+--x64 \\/);
  ...
  assert.match(workflowSource, /Validate Intel macOS DMG bundle signing/);
  assert.match(workflowSource, /hdiutil attach -nobrowse -readonly -mountroot "\$\{mount_dir\}" "\$\{dmg_path\}"/);
  assert.match(workflowSource, /Electron Framework main binary is missing from DMG/);
  assert.match(workflowSource, /codesign --verify --deep --strict --verbose=2 "\$\{mounted_app\}"/);
  assert.match(workflowSource, /spctl -a -vv -t exec "\$\{mounted_app\}"/);
  assert.match(workflowSource, /xcrun stapler validate "\$\{mounted_app\}"/);
