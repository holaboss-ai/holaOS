...
test("desktop runtime binding keeps the existing binding on any non-forced refresh failure", async () => {
  ...
  assert.match(
    ensureBindingFunction,
    /if \(canUseExistingBindingOnRefreshFailure\) \{[\s\S]*markTransientRuntimeBindingRefreshFailure\(userId\);[\s\S]*keep_existing_binding_after_refresh_failure:/,
  );
  assert.match(
    ensureBindingFunction,
    /if \(canUseExistingBindingOnRefreshFailure\) \{[\s\S]*return;\s*\}[\s\S]*await clearRuntimeBindingSecrets\(`\$\{reason\}:provision_failed`\);/,
  );
  assert.doesNotMatch(
    ensureBindingFunction,
    /canUseExistingBindingOnRefreshFailure &&\s*isTransientRuntimeError\(error\)/,
  );
});
