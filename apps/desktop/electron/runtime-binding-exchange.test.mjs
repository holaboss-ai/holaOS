...
test("desktop runtime no longer backfills Codex defaults into runtime config", async () => {
  const source = await readFile(mainSourcePath, "utf8");

  assert.doesNotMatch(source, /function withProviderDefaultModels\(/);
  assert.doesNotMatch(source, /function runtimeDocumentProviderModelIds\(/);
  assert.doesNotMatch(source, /function withOpenAiCodexProviderState\(/);
  assert.doesNotMatch(source, /async function syncOpenAiCodexDefaultsToRuntimeConfigIfNeeded\(/);
  assert.doesNotMatch(source, /await syncOpenAiCodexDefaultsToRuntimeConfigIfNeeded\(\);/);
});
