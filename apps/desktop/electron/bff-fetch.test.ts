...
test("retries once on a transient network error, then succeeds", async () => {
  let calls = 0;
  const h = harness({ retryDelayMs: 1 }, () => {
    calls += 1;
    if (calls === 1) {
...
test("does not retry a non-transient error (single attempt)", async () => {
  const h = harness({ retryDelayMs: 1 }, () => {
...
test("surfaces the undici cause in the error log and thrown message", async () => {
  const h = harness({ retryDelayMs: 1 }, () => {
    throw transientError("EAI_AGAIN");
  });
  await assert.rejects(
  ...
  assert.ok(errorEvent);
  if (errorEvent && errorEvent.event === "bff_fetch.error") {
    assert.equal(errorEvent.cause, "EAI_AGAIN");
    assert.equal(errorEvent.attempts, 2);
  }
});
