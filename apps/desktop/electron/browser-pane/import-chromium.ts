...
export function chromiumFamilyUserDataRootCandidates(
  ...
  switch (process.platform) {
    case "darwin":
      ...
      if (browser === "arc") {
        // Arc on macOS uses a chromium-style `User Data` subfolder.
        // Falls back to the bare app dir for any older / unusual install.
        return [appSupport("Arc", "User Data"), appSupport("Arc")];
      }
      if (browser === "edge") {
      ...
      if (browser === "dia") {
        // Dia (The Browser Company, Arc's sibling) — try the chromium-style
        // `User Data` subfolder first, then the bare app dir.
        return [appSupport("Dia", "User Data"), appSupport("Dia")];
      }
    ...
    case "win32":
      ...
      if (browser === "dia") {
        return [
          path.join(localAppData, "Dia", "User Data"),
          path.join(localAppData, "TheBrowserCompany", "Dia", "User Data"),
        ];
      }
    ...
    case "linux":
      ...
      if (browser === "edge") {
        return [path.join(configHome, "microsoft-edge")];
      }
      if (browser === "brave") {
        return [path.join(configHome, "BraveSoftware", "Brave-Browser")];
      }
      if (browser === "dia") {
        return [path.join(configHome, "Dia"), path.join(configHome, "dia")];
      }
