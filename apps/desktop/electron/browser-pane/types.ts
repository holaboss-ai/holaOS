...
/**
 * Source kinds the import flow accepts from the renderer. The Chromium-family
 * members each have their OWN macOS Keychain "Safe Storage" key, so an imported
 * profile only decrypts its cookies/logins when relaunched under the SAME
 * browser binary (see main.ts `chromiumFamilyBinaryCandidates` / the launcher).
 */
