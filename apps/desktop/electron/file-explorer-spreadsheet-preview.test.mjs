...
test("desktop file preview supports tabular spreadsheet kinds", async () => {
  ...
  assert.match(source, /const nodeRequire = createRequire\(__filename\);[\s\S]*const ExcelJS = nodeRequire\("exceljs"\) as ExcelJSModule;/);
  ...
  assert.match(source, /const hyperlinkValue = readWorksheetCellHyperlink\(cell\);/);
  assert.match(source, /normalizePreviewTableLinkTarget\(hyperlinkValue \?\? displayValue\),/);
  assert.match(source, /links,\s*totalRows: allRows\.length,/);
  assert.match(source, /if \(hyperlink\) \{\s*cell\.value = \{ text: value, hyperlink \};\s*\} else \{\s*cell\.value = value;\s*\}/);
...
test("desktop file explorer protects managed workspace paths without constraining file access", async () => {
  const source = await readFile(mainSourcePath, "utf8");
  ...
  assert.match(source, /await resolveLocalWorkspaceRoot\(normalizedWorkspaceId\)/);
  assert.match(source, /const relativePath = path\.relative\(rootPath, targetPath\);/);
  // Neither reads NOR mutations are bounded by the workspace root anymore: the
  // runtime writes outputs outside it, the IPC handler is gated to the trusted
  // main renderer, and the in-process runtime already has full fs access, so a
  // containment constraint bought no security while breaking real access. No
  // "escapes workspace root" throw should survive. The only remaining guards
  // protect the root NODE itself and managed files (workspace.yaml / AGENTS.md
  // / skills) from being destroyed via the explorer.
  assert.doesNotMatch(source, /escapes workspace root/);
  ...
  assert.match(source, /kind === "directory"\s*\? "New Folder"\s*: "Untitled\.txt"/);
  assert.match(
    source,
  ...
  assert.match(
    source,
    /"fs:createPath"[\s\S]*parentPath: string \| null \| undefined,[\s\S]*kind: FileSystemCreateKind,[\s\S]*createExplorerPath\(\s*parentPath,\s*kind,\s*workspaceId,\s*extensionHint,\s*desiredName,?\s*\)/,
  );
...
test("desktop preload exposes file preview watch subscriptions and change events", async () => {
  ...
  assert.match(
    source,
    /createPath: \(\s*parentPath: string \| null \| undefined,\s*kind: "file" \| "directory",\s*workspaceId\?: string \| null,\s*extensionHint\?: string \| null,\s*desiredName\?: string \| null,\s*\) =>[\s\S]*ipcRenderer\.invoke\(\s*"fs:createPath",\s*parentPath,\s*kind,\s*workspaceId,\s*extensionHint,\s*desiredName,\s*\) as Promise<FileSystemMutationPayload>/,
  );
  assert.match(source, /type ExplorerExternalImportEntryPayload =/);
  assert.match(
    source,
