...
export type StepResult<TOut = {}> =
  | { ok: true; data?: TOut; externalId?: string; nextState?: string }
  | { fail: BridgeError | { kind: "error"; code: string; message: string } }

export type ActionRunResult<TOut = unknown> =
...
export interface ActionDef<TRow, States extends StateTuple, I = {}> {
   ...
   * `null` = side-effect action; row.status is NOT changed unless the action
   * runtime returns an explicit `nextState`.
   */
  toState: States[number] | null
  /**
   * Optional action-scoped failure state override. Falls back to the
   * resource-level failedState when omitted.
   */
  failedState?: States[number]
  schema?: ZodSchema<I>
...
export interface AppHandle {
  ...
  createRow<TSchema extends ZodTypeAny, States extends StateTuple>(
    resource: ResourceHandle<TSchema, States>,
    data: Infer<TSchema>,
    options?: { status?: States[number]; turnContext?: TurnContext | null },
  ): RowOf<TSchema>
  getRow<TSchema extends ZodTypeAny, States extends StateTuple>(
    resource: ResourceHandle<TSchema, States>,
    rowId: string,
  ): RowOf<TSchema> | null
  listRows<TSchema extends ZodTypeAny, States extends StateTuple>(
    resource: ResourceHandle<TSchema, States>,
  ): RowOf<TSchema>[]
  runAction<TInput = Record<string, unknown>, TOutput = unknown>(params: {
    actionName: string
    rowId: string
    input?: TInput
    bridge: BridgeClient
  ...
  reverseAction<TOutput = unknown>(params: {
    actionName: string
    rowId: string
    bridge: BridgeClient
    turnContext?: TurnContext | null
