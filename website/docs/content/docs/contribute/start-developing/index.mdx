...
- The Electron user-data directory controls the embedded sandbox root and `runtime.log`. In dev, Electron resolves `HOLABOSS_DESKTOP_USER_DATA_PATH` first, then `HOLABOSS_DESKTOP_USER_DATA_DIR` from the shell or `desktop/.env`, and otherwise falls back to `holaboss-local-dev`.
