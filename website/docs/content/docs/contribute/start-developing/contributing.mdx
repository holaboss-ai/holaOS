...
| SDK or bridge contracts | `sdk/app-sdk/**`, `sdk/bridge/**` | Run the matching `build`, `typecheck`, and `test` scripts for the package you changed, for example `bun --filter=@holaboss/bridge run typecheck` and `bun --filter=@holaboss/bridge run test` |
