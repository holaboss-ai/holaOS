...
This page is the configuration reference for operators and builders. For the Electron IPC path, runtime model-catalog refresh flow, queued `thinking_value` handoff, and other execution internals, continue into [Desktop Internals](/docs/contribute/desktop/internals) and [Runtime APIs](/docs/contribute/runtime/apis).
...
| `openai_compatible` | OpenAI-style APIs | Best when you already have an OpenAI-style endpoint. Covers OpenAI, Ollama, MiniMax, and many self-hosted compatible APIs. |
...
The desktop settings UI (`Settings -> Model Providers`) lets you connect holaOS Proxy, OpenAI, OpenAI Codex, Anthropic, OpenRouter, Gemini, Ollama, or MiniMax without editing files manually.
...
- connect a provider such as holaOS Proxy, OpenAI, OpenAI Codex, Anthropic, OpenRouter, Gemini, Ollama, or MiniMax
- enter the API key and use the built-in provider defaults or edit the model list for that provider
- use the `Background tasks` panel to choose one connected provider and model for recall selection, finalization, and other background LLM tasks
...
- Direct providers use the shipped fallback catalog in `apps/desktop/shared/model-catalog.ts`.
...
| `openai_codex/gpt-5.4` | `none`, `low`, `medium`, `high`, `xhigh` |
| `openai_codex/gpt-5.5` | `none`, `low`, `medium`, `high`, `xhigh` |
| `openai_codex/gpt-5.3-codex` | `low`, `medium`, `high`, `xhigh` |
...
| `minimax_direct/MiniMax-M3` | `adaptive`, `disabled` |
| `minimax_direct/MiniMax-M2.7` | `always_on` |
...
- Google-compatible direct providers use the Gemini compatibility root, for example `https://generativelanguage.googleapis.com/v1beta/openai`
...
### MiniMax direct endpoints

The built-in `minimax_direct` provider uses the international OpenAI-compatible endpoint. A structured provider catalog can expose every supported region and protocol without adding new configuration fields:

| Region | Protocol | Provider kind | Base URL |
| --- | --- | --- | --- |
| International | OpenAI-compatible | `openai_compatible` | `https://api.minimax.io/v1` |
| International | Anthropic-compatible | `anthropic_native` | `https://api.minimax.io/anthropic` |
| China | OpenAI-compatible | `openai_compatible` | `https://api.minimaxi.com/v1` |
| China | Anthropic-compatible | `anthropic_native` | `https://api.minimaxi.com/anthropic` |

Use an Anthropic Base URL that ends at `/anthropic`; the client appends `/v1/messages`. OpenAI-compatible clients append `/chat/completions` to the `/v1` Base URL.

```json
{
  "runtime": {
    "default_provider": "minimax_global_openai",
    "default_model": "minimax_global_openai/MiniMax-M3"
  },
  "providers": {
    "minimax_global_openai": {
      "kind": "openai_compatible",
      "base_url": "https://api.minimax.io/v1",
      "api_key": "<MINIMAX_API_KEY>"
    },
    "minimax_global_anthropic": {
  ...
  "models": {
    "minimax_global_openai/MiniMax-M3": {
      "provider": "minimax_global_openai",
      "model": "MiniMax-M3"
    },
    "minimax_global_anthropic/MiniMax-M3": {
      "provider": "minimax_global_anthropic",
      "model": "MiniMax-M3"
    },
    "minimax_cn_openai/MiniMax-M3": {
      "provider": "minimax_cn_openai",
      "model": "MiniMax-M3"
    },
    "minimax_cn_anthropic/MiniMax-M3": {
      "provider": "minimax_cn_anthropic",
      "model": "MiniMax-M3"
    }
  }
}
...
- `runtime.background_tasks.provider`
  - provider for recall selection, finalization, and other background LLM tasks
