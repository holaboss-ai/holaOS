...
If a route change also changes durable records, continue to [Runtime State Store](/docs/contribute/runtime/state-store). If it changes how a run is compiled or which tools/context the harness receives, continue to [Run Compilation](/docs/contribute/runtime/run-compilation).
...
- The embedded desktop runtime binds to `127.0.0.1:<derived-port>`. `apps/desktop/electron/main.ts` resolves that port from `HOLABOSS_RUNTIME_API_PORT` or, when unset, from a hash of the Electron `userData` path. Port `5060` is rejected, and `5160` is only the bootstrap fallback before per-profile derivation runs.

Use the right port before you assume a route is broken.

The examples below use `RUNTIME_PORT` as a placeholder for the desktop runtime port when you are debugging the embedded path.
...
| Memory and post-run system state | `/api/v1/memory/*`, `/api/v1/memory-update-proposals*`, `/api/v1/task-proposals*` | `memory.ts`, `user-memory-proposals.ts`, `turn-memory-writeback.ts`, and queue or evolve workers |
...
```bash
RUNTIME_PORT=39160
curl -X POST http://127.0.0.1:${RUNTIME_PORT}/api/v1/capabilities/runtime-tools/reports \
...
RUNTIME_PORT=39160
curl -N \
  "http://127.0.0.1:${RUNTIME_PORT}/api/v1/agent-sessions/session-main/outputs/stream?include_history=false&stop_on_terminal=true"
```
...
curl http://127.0.0.1:${RUNTIME_PORT}/api/v1/runtime/status
```
