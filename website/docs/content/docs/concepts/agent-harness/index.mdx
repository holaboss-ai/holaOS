...
- browser bridge state when the selected session kind is allowed to use browser tools
...
Browser capability is also runtime-shaped, not host-invented.

In the current `pi` path, direct browser tools are generally surfaced to delegated `subagent` runs when the desktop browser bridge is available. The front `main_session` path usually routes browser-heavy work through delegation instead of exposing the full browser tool surface directly.

So there are two separate truths:

- the runtime may still carry browser bridge context in the prepared request
- the actual browser tool surface is session-kind dependent and currently enabled for delegated executor sessions
