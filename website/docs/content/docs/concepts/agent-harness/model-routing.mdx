...
`openai_codex` is the notable OpenAI-compatible special case on the desktop side. It still resolves through the OpenAI-compatible routing family, but it can use Codex-specific base URLs instead of the generic OpenAI `/v1` path.
