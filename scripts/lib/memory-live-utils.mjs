...
export function resolveWorkspaceDir(params) {
  ...
  for (const candidateRoot of candidateRoots) {
    const candidateWorkspaceDir = path.join(candidateRoot, params.workspaceId);
    if (fs.existsSync(candidateWorkspaceDir)) {
      return candidateWorkspaceDir;
    }
  }
  return path.join(candidateRoots[0], params.workspaceId);
}
...
function countTableRows(dbPath, tableNames, tableName, whereClause = "") {
  if (!tableNames.has(tableName)) {
    return 0;
  }
  const suffix = whereClause.trim().length > 0 ? ` WHERE ${whereClause}` : "";
  return Number(runSql(dbPath, `SELECT COUNT(*) FROM ${tableName}${suffix};`) || 0);
}

export function workspaceMemoryCounts(runtimeDbPathValue, controlPlaneDbPathValue = null) {
  const runtimeTableNames = sqliteTableNames(runtimeDbPathValue);
  const controlPlaneTableNames =
    controlPlaneDbPathValue && fs.existsSync(controlPlaneDbPathValue)
      ? sqliteTableNames(controlPlaneDbPathValue)
      : new Set();
  return {
    active_entities: countTableRows(runtimeDbPathValue, runtimeTableNames, "interaction_entities", "status = 'active'"),
    active_leaves: countTableRows(runtimeDbPathValue, runtimeTableNames, "interaction_leaves", "status = 'active'"),
    active_summaries: countTableRows(runtimeDbPathValue, runtimeTableNames, "interaction_summary_nodes", "status = 'active'"),
    active_semantic_nodes: countTableRows(runtimeDbPathValue, runtimeTableNames, "semantic_memory_nodes", "status = 'active'"),
    active_semantic_trees: countTableRows(
      runtimeDbPathValue,
      runtimeTableNames,
      "semantic_memory_nodes",
      "status = 'active' AND node_class = 'semantic' AND node_kind = 'tree'",
    ),
    active_semantic_relations: countTableRows(runtimeDbPathValue, runtimeTableNames, "semantic_memory_relations"),
    active_semantic_evidence_refs: countTableRows(runtimeDbPathValue, runtimeTableNames, "semantic_memory_evidence_refs"),
    active_semantic_search_docs: countTableRows(runtimeDbPathValue, runtimeTableNames, "semantic_memory_search_docs"),
    outputs: countTableRows(runtimeDbPathValue, runtimeTableNames, "outputs"),
    turn_results: countTableRows(runtimeDbPathValue, runtimeTableNames, "turn_results"),
    active_integration_trees: countTableRows(
      controlPlaneDbPathValue,
      controlPlaneTableNames,
      "integration_trees",
      "status = 'active'",
    ),
    active_integration_leaves: countTableRows(
      controlPlaneDbPathValue,
      controlPlaneTableNames,
      "integration_leaves",
      "status = 'active'",
    ),
    active_integration_summaries: countTableRows(
      controlPlaneDbPathValue,
      controlPlaneTableNames,
      "integration_summary_nodes",
      "status = 'active'",
    ),
  };
}
...
export function cleanupWorkspaceMemory(params) {
  ...
  if (!params.dryRun) {
    ...
    const controlPlaneTableNames = sqliteTableNames(cpDbPath);
    ...
    ].filter((statement) => runtimeTableNames.has(statement.match(/^DELETE FROM ([^;]+)/)?.[1] ?? ""));
    const controlPlaneDeletes = [
    ...
    ].filter((statement) => controlPlaneTableNames.has(statement.match(/^DELETE FROM ([^;]+)/)?.[1] ?? ""));
    const metadataDeletes = runtimeTableNames.has("workspace_runtime_metadata")
      ? [
          "DELETE FROM workspace_runtime_metadata WHERE key LIKE 'interaction_memory_batch_%';",
          "DELETE FROM workspace_runtime_metadata WHERE key LIKE 'workspace_artifact_relation_backfill_%';",
          "DELETE FROM workspace_runtime_metadata WHERE key LIKE 'workspace_artifact_source_turn_input_position_backfill_%';",
          "DELETE FROM workspace_runtime_metadata WHERE key LIKE 'workspace_attachment_artifact_tree_backfill_%';",
          "DELETE FROM workspace_runtime_metadata WHERE key LIKE 'workspace_image_url_artifact_tree_backfill_%';",
          "DELETE FROM workspace_runtime_metadata WHERE key LIKE 'workspace_output_artifact_tree_backfill_%';",
          "DELETE FROM workspace_runtime_metadata WHERE key LIKE 'workspace_tool_result_artifact_tree_backfill_%';",
        ]
      : [];
    execFileSync(
      "sqlite3",
    ...
    fs.rmSync(semanticMemoryDir(params.workspaceDir), { recursive: true, force: true });
    fs.rmSync(evolveMemoryDir(params.workspaceDir), { recursive: true, force: true });
    if (params.includeSessionHistory === true) {
      fs.rmSync(runtimeContinuityDir(params.workspaceDir), { recursive: true, force: true });
    ...
    fs.mkdirSync(path.join(semanticMemoryDir(params.workspaceDir), "interaction"), { recursive: true });
    fs.mkdirSync(path.join(semanticMemoryDir(params.workspaceDir), "workspace"), { recursive: true });
