...
test("Pi runtime tools resolve the full runtime-agent tool list", async () => {
  // The per-tool execute path (request headers, result envelope) is covered by
  // the memory_retrieve / web_search tests below; this guards that the resolved
  // set matches the production tool ids. (The old onboarding example tool was
  // removed with the onboarding feature in the workspace-removal refactor.)
  const fetchImpl: typeof fetch = async (input) => {
    if (String(input).endsWith("/api/v1/capabilities/runtime-tools")) {
...
test("Pi runtime tools fall back to node http when no fetch implementation is provided", async () => {
  ...
  const server = http.createServer((request, response) => {
    ...
    if (request.method === "POST" && url === "/api/v1/capabilities/runtime-tools/memory/retrieve") {
      let body = "";
      ...
      request.on("end", () => {
        ...
        response.writeHead(200, { "content-type": "application/json; charset=utf-8" });
        response.end(JSON.stringify({ tool_id: "memory_retrieve", hits: [{ title: "Orchid customer escalation contact" }] }));
      });
      return;
    }
  ...
  try {
    ...
    const result = await memoryTool.execute(
      "call-1",
      { query: "Orchid customer escalation contact", mode: "mixed", max_results: 10 },
      undefined,
      undefined,
      {} as never
    );
    ...
    assert.deepEqual(requests, [
      {
        ...
        sessionId: "session-main",
        selectedModel: "openai/gpt-5.4",
        body: JSON.stringify({ query: "Orchid customer escalation contact", mode: "mixed", max_results: 10 }),
      },
    ]);
    assert.equal(result.content[0]?.type, "text");
    assert.equal(
