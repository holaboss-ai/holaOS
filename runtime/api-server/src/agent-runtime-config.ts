...
import {
  ...
  type AgentToolNodeContext,
} from "./agent-runtime-prompt.js";
import {
  renderRecalledMemoryPromptSection,
...
export interface AgentRuntimeConfigCliRequest {
  ...
  // Org-owned sessions: the session's org (billing), threaded from the exec
  // context so the run bills the session's org, not the live runtime-config org.
  runtime_exec_org_id?: string | null;
  ...
  composio_inline_tool_refs?: Array<{
    name: string;
    toolkit_slug: string;
    tool_slug: string;
  ...
  tool_node_context?: {
    ...
    upstream_assistant_text?: string | null;
  } | null;
  workflow_owned_subagent?: boolean | null;
}
...
function agentRoleHeaderValue(
  sessionKind: string | null | undefined,
): string {
  const normalized = typeof sessionKind === "string" ? sessionKind.trim().toLowerCase() : "";
  if (
    !normalized ||
...
function resolveModelClientConfig(
  ...
  // Group every model call in ONE turn under a single run_id (the usage log groups by it). The
  // pi-brain passes an explicit run_id (= `${workspaceId}:${sessionId}:${inputId}`, per
  // claimedInputRunId); the in-turn recall/embedding calls don't pass one, but they DO carry the
  // turn's real workspace/session/input — so derive the SAME composite and they fold into the turn.
  // Background memory maintenance carries synthetic session/input (e.g. `memory-retrieve:<ws>`), so
  // it derives a DIFFERENT composite and correctly stays as its own row.
  const explicitRunId = request.runtime_exec_run_id?.trim() ?? "";
  const workspaceId = request.workspace_id?.trim() ?? "";
  const sessionId = request.session_id?.trim() ?? "";
  const inputId = request.input_id?.trim() ?? "";
  const runId =
    explicitRunId ||
...
function selectedRuntimeOutputSchema(request: AgentRuntimeConfigCliRequest): {
  ...
  // tool_node sessions force a structured-output contract derived from
  // the target object's field schema. The agent emits the LLM-territory
  // delta as JSON; the workflow runtime merges it with the deterministic
  // prefill and calls the underlying capability.
  if (
    (request.session_kind ?? "").trim().toLowerCase() === "tool_node" &&
    request.tool_node_context?.output_schema &&
    typeof request.tool_node_context.output_schema === "object"
  ) {
    return {
      outputSchemaMemberId: `tool_node:${request.tool_node_context.tool_kind}`,
      outputFormat: {
        type: "json_schema",
        schema: request.tool_node_context.output_schema as Record<string, unknown>,
...
export function projectAgentRuntimeConfig(
  ...
  const capabilityManifest = buildAgentCapabilityManifest({
    ...
    composioInlineToolRefs: directMcpDisabledForSession(request.session_kind ?? null)
      ? []
      : (request.composio_inline_tool_refs ?? []).map((entry) => ({
          name: entry.name,
  ...
  const delegatedCapabilityManifest =
    ...
    Array.isArray(request.delegated_resolved_mcp_tool_refs)
      ? buildAgentCapabilityManifest({
          ...
          toolServerIdMap: request.tool_server_id_map ?? null,
          composioInlineToolRefs: (request.composio_inline_tool_refs ?? []).map(
            (entry) => ({
              name: entry.name,
              toolkit_slug: entry.toolkit_slug,
              tool_slug: entry.tool_slug,
      ...
      : null;
  const toolNodeContextForPrompt: AgentToolNodeContext | null =
    request.tool_node_context && request.tool_node_context.tool_kind
      ? {
          toolKind: request.tool_node_context.tool_kind,
          draftPayload:
            (request.tool_node_context.draft_payload as
              | Record<string, unknown>
              | null
              | undefined) ?? null,
          outputSchema:
            (request.tool_node_context.output_schema as
              | Record<string, unknown>
              | null
              | undefined) ?? null,
          upstreamSummary: request.tool_node_context.upstream_summary ?? null,
          upstreamAssistantText:
            request.tool_node_context.upstream_assistant_text ?? null,
        }
      : null;
  const promptComposition = composeAgentPrompt(request.agent.prompt, {
    ...
    harnessId: request.harness_id ?? null,
    recalledMemorySection: request.recalled_memory_context
      ? renderRecalledMemoryPromptSection(request.recalled_memory_context)
      : null,
    ...
    toolNodeContext: toolNodeContextForPrompt,
    workflowOwnedSubagent: request.workflow_owned_subagent === true,
  });
