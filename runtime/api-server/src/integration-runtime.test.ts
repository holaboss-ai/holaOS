...
test("injects workspace api url and legacy token for a single active binding", async () => {
  ...
  const result = await resolveIntegrationRuntime({
    store,
    workspaceId: workspace.id,
    appId: "gmail",
...
test("suppresses legacy token when multiple active platform-backed bindings resolve", async () => {
  const root = makeTempDir("hb-integration-runtime-");
  const store = new RuntimeStateStore({
    dbPath: path.join(root, "runtime.db"),
    workspaceRoot: path.join(root, "workspace")
...
test("skips inactive connections when building compatibility env", async () => {
  const root = makeTempDir("hb-integration-runtime-");
  const store = new RuntimeStateStore({
    dbPath: path.join(root, "runtime.db"),
    workspaceRoot: path.join(root, "workspace")
...
test("prefers an app-specific binding override over a workspace default binding", async () => {
  const root = makeTempDir("hb-integration-runtime-");
  const store = new RuntimeStateStore({
    dbPath: path.join(root, "runtime.db"),
    workspaceRoot: path.join(root, "workspace")
...
test("checkIntegrationReadiness returns integration_not_bound when no binding exists", async () => {
  ...
  const readiness = await checkIntegrationReadiness({
    store,
    workspaceId: workspace.id,
    appId: "gmail",
    resolvedApp: createSingleIntegrationResolvedApp()
...
test("checkIntegrationReadiness returns integration_needs_reauth for inactive connection", async () => {
  const root = makeTempDir("hb-integration-readiness-");
  const store = new RuntimeStateStore({
    dbPath: path.join(root, "runtime.db"),
    workspaceRoot: path.join(root, "workspace")
...
test("checkIntegrationReadiness returns ready when all required integrations are bound and active", async () => {
  const root = makeTempDir("hb-integration-readiness-");
  const store = new RuntimeStateStore({
    dbPath: path.join(root, "runtime.db"),
    workspaceRoot: path.join(root, "workspace")
...
test("checkIntegrationReadiness accepts provider-key bindings for legacy custom manifest keys", async () => {
  const root = makeTempDir("hb-integration-readiness-");
  const store = new RuntimeStateStore({
    dbPath: path.join(root, "runtime.db"),
    workspaceRoot: path.join(root, "workspace")
...
test("checkIntegrationReadiness skips optional integrations", async () => {
  const root = makeTempDir("hb-integration-readiness-");
  const store = new RuntimeStateStore({
    dbPath: path.join(root, "runtime.db"),
    workspaceRoot: path.join(root, "workspace")
