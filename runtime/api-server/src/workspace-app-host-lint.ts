...
export interface ProviderEffectManifestViolation {
  file: string;
  line: number;
  provider: string;
  snippet: string;
...
export function findProviderEffectManifestViolations(
  appDir: string,
  declaredProviders: readonly string[],
): ProviderEffectManifestViolation[] {
  const violations: ProviderEffectManifestViolation[] = [];
  ...
  if (declared.size === 0) {
    // Still scan — every literal provider usage should be declared.
  }
  ...
  const seenFiles = new Set<string>();
  const providerEffectPattern =
    /providerEffectAction\s*\(\s*\{[\s\S]{0,600}?provider\s*:\s*["'`]([^"'`]+)["'`]/gi;
  ...
  for (const root of candidateRoots) {
    for (const file of walkSourceFiles(root)) {
      ...
      providerEffectPattern.lastIndex = 0;
      for (const match of contents.matchAll(providerEffectPattern)) {
        const provider = (match[1] ?? "").trim();
        if (!provider) continue;
        if (declared.has(provider.toLowerCase())) continue;
        const matchIndex = typeof match.index === "number" ? match.index : contents.indexOf(match[0]);
        const safeIndex = matchIndex >= 0 ? matchIndex : 0;
        const upTo = contents.slice(0, safeIndex);
        const line = upTo.split("\n").length;
        const lineText = contents.split("\n")[line - 1] ?? "";
        violations.push({
          file: path.relative(appDir, file) || file,
...
export function formatProviderEffectManifestLintError(
  violations: ProviderEffectManifestViolation[],
): string {
  const lines: string[] = [
    "app source uses providerEffectAction(...) with providers that are not declared in app.runtime.yaml integrations.",
  ];
  for (const violation of violations.slice(0, 5)) {
    lines.push(
      `  ${violation.file}:${violation.line} — providerEffectAction provider '${violation.provider}'`,
    );
    if (violation.snippet) {
      lines.push(`      ${violation.snippet}`);
    }
  ...
  lines.push(
    "Fix: add a matching `integrations:` entry to app.runtime.yaml for every providerEffectAction({ provider: \"...\" }) call, and keep the provider id identical to the runtime integration catalog entry.",
  );
  return lines.join("\n");
}
