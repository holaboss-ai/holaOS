...
type IntegrationSemanticCategory = Extract<SemanticMemoryCategory, "integration" | "workspace">;
const INTEGRATION_SEMANTIC_LEAF_BACKFILL_KEY_PREFIX =
  "integration_semantic_leaf_backfill_v1:";
const INTEGRATION_SEMANTIC_ROOT_BACKFILL_KEY_PREFIX =
  "integration_semantic_root_backfill_v1:";
const ACTIVE_WORKSPACE_INTEGRATION_CATEGORY: IntegrationSemanticCategory = "workspace";

function normalizeIntegrationSemanticWorkspaceId(workspaceId?: string | null): string | null {
  return typeof workspaceId === "string" && workspaceId.trim().length > 0
    ? workspaceId.trim()
    : null;
}

export function writableIntegrationSemanticCategory(
  workspaceId?: string | null,
): IntegrationSemanticCategory {
  return normalizeIntegrationSemanticWorkspaceId(workspaceId) ? "workspace" : "integration";
}

export function readableIntegrationSemanticCategory(params: {
  store: RuntimeStateStore;
  workspaceId?: string | null;
  treeId: string;
  cache?: Map<string, IntegrationSemanticCategory>;
}): IntegrationSemanticCategory {
  const workspaceId = normalizeIntegrationSemanticWorkspaceId(params.workspaceId);
  if (!workspaceId) {
    return "integration";
  }
  ...
  const category: IntegrationSemanticCategory = hasWorkspaceSemanticState()
    ? "workspace"
...
function integrationSemanticLeafBackfillMetadataKey(treeId: string): string {
  return `${INTEGRATION_SEMANTIC_LEAF_BACKFILL_KEY_PREFIX}${treeId}`;
}

function integrationSemanticRootBackfillMetadataKey(treeId: string): string {
  return `${INTEGRATION_SEMANTIC_ROOT_BACKFILL_KEY_PREFIX}${treeId}`;
}
...
export function countSummaryLikeSemanticIntegrationNodes(params: {
  ...
  return params.store.listSemanticMemoryNodes({
    category: ACTIVE_WORKSPACE_INTEGRATION_CATEGORY,
...
export function queueIntegrationTreeRebuild(params: {
  ...
  const key = `${workspaceId}:${params.treeId.trim()}`;
...
type SemanticEvidenceRef = ReturnType<RuntimeStateStore["listSemanticMemoryEvidenceRefs"]>[number];
...
function semanticLeafRecordFromNode(params: {
  node: ReturnType<RuntimeStateStore["listSemanticMemoryNodes"]>[number];
  evidenceRef: SemanticEvidenceRef | null;
}): IntegrationLeafRecord | null {
  const leafId = (params.node.sourceLeafId ?? "").trim();
  if (!leafId) {
    return null;
  }
  const metadata = params.evidenceRef?.metadata ?? params.node.metadata ?? {};
  const tags = Array.isArray(metadata.tags)
    ? metadata.tags.filter((value): value is string => typeof value === "string")
    : [];
  return {
    workspaceId: params.node.workspaceId,
    leafId,
    treeId: params.node.treeId,
    subjectKey:
      typeof metadata.subject_key === "string" && metadata.subject_key.trim().length > 0
        ? metadata.subject_key
        : leafId,
    entityKey:
      typeof metadata.entity_key === "string" && metadata.entity_key.trim().length > 0
        ? metadata.entity_key
        : null,
    entityLabel:
      typeof metadata.entity_label === "string" && metadata.entity_label.trim().length > 0
        ? metadata.entity_label
        : null,
    branchKey:
      typeof metadata.branch_key === "string" && metadata.branch_key.trim().length > 0
        ? metadata.branch_key
        : null,
    branchLabel:
      typeof metadata.branch_label === "string" && metadata.branch_label.trim().length > 0
        ? metadata.branch_label
        : null,
    path:
      (typeof metadata.path === "string" && metadata.path.trim().length > 0
        ? metadata.path
        : typeof params.node.metadata.evidence_path === "string" && params.node.metadata.evidence_path.trim().length > 0
          ? params.node.metadata.evidence_path
          : params.node.path),
    title: params.node.title,
    summary: params.node.summary,
    fingerprint:
      typeof metadata.fingerprint === "string" && metadata.fingerprint.trim().length > 0
        ? metadata.fingerprint
        : params.node.bodySha256,
    bodySha256:
      typeof metadata.body_sha256 === "string" && metadata.body_sha256.trim().length > 0
        ? metadata.body_sha256
        : params.node.bodySha256,
    tags,
    sourceType: params.evidenceRef?.sourceType ?? (
      typeof params.node.metadata.source_type === "string"
        ? params.node.metadata.source_type
        : null
    ),
    sourceEventId: params.evidenceRef?.sourceEventId ?? (
      typeof params.node.metadata.source_event_id === "string"
        ? params.node.metadata.source_event_id
        : null
    ),
    sourceMessageId: params.evidenceRef?.sourceMessageId ?? null,
    externalObjectId: params.evidenceRef?.externalObjectId ?? (
      typeof params.node.metadata.external_object_id === "string"
        ? params.node.metadata.external_object_id
        : null
    ),
    externalObjectType: params.evidenceRef?.externalObjectType ?? (
      typeof params.node.metadata.external_object_type === "string"
        ? params.node.metadata.external_object_type
        : null
    ),
    admissionConfidence: null,
    observedAt: params.evidenceRef?.observedAt ?? params.node.observedAt ?? null,
    supersedesLeafId: null,
    supersededAt: null,
    status: params.node.status === "archived" ? "archived" : "active",
    createdAt: params.evidenceRef?.createdAt ?? params.node.createdAt,
    updatedAt: params.evidenceRef?.updatedAt ?? params.node.updatedAt,
  };
}

function integrationLeafViewFromSemanticState(params: {
  nodes: ReturnType<RuntimeStateStore["listSemanticMemoryNodes"]>;
  evidenceRefs: SemanticEvidenceRef[];
}): IntegrationLeafRecord[] {
  const evidenceByNodeId = new Map<string, SemanticEvidenceRef>();
  for (const ref of params.evidenceRefs) {
    if (!evidenceByNodeId.has(ref.nodeId)) {
      evidenceByNodeId.set(ref.nodeId, ref);
    }
  }
  return params.nodes
    .map((node) =>
      semanticLeafRecordFromNode({
        node,
        evidenceRef: evidenceByNodeId.get(node.nodeId) ?? null,
      })
    )
    .filter((leaf): leaf is IntegrationLeafRecord => Boolean(leaf))
    .sort((left, right) =>
...
export function listActiveIntegrationLeafView(params: {
  store: RuntimeStateStore;
  workspaceId: string;
  tree: IntegrationTreeRecord;
  categoryCache?: Map<string, IntegrationSemanticCategory>;
}): IntegrationLeafRecord[] {
  const semanticLeafNodes = params.store.listSemanticMemoryNodes({
    category: ACTIVE_WORKSPACE_INTEGRATION_CATEGORY,
    workspaceId: params.workspaceId,
  ...
  return integrationLeafViewFromSemanticState({
    nodes: semanticLeafNodes,
    evidenceRefs: params.store.listSemanticMemoryEvidenceRefs({
      category: ACTIVE_WORKSPACE_INTEGRATION_CATEGORY,
      workspaceId: params.workspaceId,
      treeId: params.tree.treeId,
      limit: 20_000,
...
export function migrateLegacyWorkspaceIntegrationMemory(params: {
  store: RuntimeStateStore;
  workspaceId: string;
  treeId?: string | null;
  tree?: IntegrationTreeRecord | null;
  includeRoot?: boolean;
}): { treesMigrated: number; leavesMigrated: number } {
  ...
  const candidateTrees = requestedTreeId
    ? (() => {
      if (params.tree && params.tree.treeId === requestedTreeId) {
        return [params.tree];
      }
      ...
      if (typeof (params.store as { listIntegrationTrees?: unknown }).listIntegrationTrees !== "function") {
        return [];
      }
      const legacyTree = params.store.listIntegrationTrees({
        workspaceId: params.workspaceId,
        status: "active",
        limit: 10_000,
        offset: 0,
      }).find((tree) => tree.treeId === requestedTreeId) ?? null;
      if (legacyTree) {
        return [legacyTree];
      }
      return [];
  ...
  const includeRoot = params.includeRoot ?? true;
  for (const legacyTree of candidateTrees) {
    // workspace-removal Piece 5.7: the integration knowledge graph is now
    // account-global (control-plane-only), so raw integration trees no longer
    // carry a workspace id (`legacyTree.workspaceId` is null). They all belong to
    // the single tenant, so we only skip inactive trees and project the rest into
    // THIS workspace's semantic memory (the projection re-stamps params.workspaceId
    // via workspaceIntegrationTreeRecord below).
    if (legacyTree.status !== "active") {
      continue;
    }
    const tree = workspaceIntegrationTreeRecord({
      workspaceId: params.workspaceId,
      treeId: legacyTree.treeId,
      provider: legacyTree.provider,
      accountNamespace: legacyTree.accountNamespace,
      accountDisplayName: legacyTree.accountDisplayName,
      slug: legacyTree.slug,
      summary: legacyTree.summary,
      status: legacyTree.status,
      createdAt: legacyTree.createdAt,
      updatedAt: legacyTree.updatedAt,
    });
    const rootBackfillComplete = params.store.getWorkspaceRuntimeMetadata({
      workspaceId: params.workspaceId,
      key: integrationSemanticRootBackfillMetadataKey(tree.treeId),
    }) === "true";
    if (includeRoot && !rootBackfillComplete && canProjectSemanticRoot) {
      upsertWorkspaceIntegrationSemanticRoot({
        store: params.store,
        tree,
      });
      treesMigrated += 1;
    }
    const leafBackfillComplete = params.store.getWorkspaceRuntimeMetadata({
      workspaceId: params.workspaceId,
      key: integrationSemanticLeafBackfillMetadataKey(tree.treeId),
    }) === "true";
    if (!leafBackfillComplete) {
      const semanticCategory: IntegrationSemanticCategory = "workspace";
      const existingLeafIds = new Set(
        params.store.listSemanticMemoryNodes({
          ...
          limit: 10_000,
          offset: 0,
        }).map((node) => (node.sourceLeafId ?? "").trim()).filter(Boolean),
      );
      for (const legacyLeaf of params.store.listIntegrationLeaves({
        treeId: tree.treeId,
        status: "active",
        limit: 10_000,
        offset: 0,
      })) {
        if (existingLeafIds.has(legacyLeaf.leafId)) {
          continue;
        }
        upsertWorkspaceIntegrationSemanticLeafProjection({
          store: params.store,
          workspaceId: params.workspaceId,
          tree,
          leaf: legacyLeaf,
  ...
  if (!requestedTreeId) {
    params.store.setWorkspaceRuntimeMetadata({
      workspaceId: params.workspaceId,
      key: "integration_semantic_root_backfill_v1_complete",
      value: "true",
    });
  }
  return { treesMigrated, leavesMigrated };
}
...
function legacySemanticIntegrationTreeDir(workspaceRoot: string, slug: string): string {
...
function semanticIntegrationTreeBaseSegments(treeSlug: string): string[] {
  return ["semantic", "workspace", "systems", "integrations", treeSlug];
}
...
function appendSemanticLeafNode(params: {
  ...
  params.drafts.push({
    ...
    metadata: {
      ...
      branch_label: params.leaf.branchLabel,
      entity_key: params.leaf.entityKey,
      entity_label: params.leaf.entityLabel,
      ...
      body_sha256: params.leaf.bodySha256,
...
type SemanticIntegrationEvidenceRef = {
  nodeId: string;
  refId: string;
...
function semanticNodeBody(params: {
  ...
  const lines = [
    ...
    `- Account: ${params.tree.accountDisplayName}`,
    ...
    const summary = compactWhitespace(node.summary ?? "")
      || (childCount > 0
        ? deterministicSummaryText({
            scopeLabel: `${params.tree.accountDisplayName} ${node.title}`.trim(),
            childCount,
...
function buildNotionSemanticMemoryTree(params: {
  ...
  const rootTitle = `${params.tree.accountDisplayName} Notion connection`;
  drafts.push({
    nodeId: rootNodeId,
    nodeClass: "semantic",
    nodeKind: "connection",
    path: rootPath,
    title: rootTitle,
    summary: params.tree.summary ?? `Notion connection memory for ${params.tree.accountDisplayName}.`,
  ...
  const workspaceSlug = safePathSegment(
    String(workspaceLeaf?.externalObjectId ?? params.tree.accountNamespace ?? params.tree.accountDisplayName),
    "workspace",
  );
  ...
  drafts.push({
    ...
    nodeKind: "workspace",
    path: workspacePath,
    title: workspaceLeaf?.title ?? `Notion workspace for ${params.tree.accountDisplayName}`,
    summary: workspaceLeaf?.summary ?? `Notion workspace memory for ${params.tree.accountDisplayName}.`,
...
function buildGitHubSemanticMemoryTree(params: {
  ...
  drafts.push({
    ...
    nodeKind: "connection",
    path: rootPath,
    title: `${params.tree.accountDisplayName} GitHub connection`,
    summary: params.tree.summary ?? `GitHub connection memory for ${params.tree.accountDisplayName}.`,
    observedAt: params.tree.updatedAt,
    metadata: {
      provider: params.tree.provider,
      account_namespace: params.tree.accountNamespace,
...
function buildGmailSemanticMemoryTree(params: {
  ...
  drafts.push({
    ...
    nodeKind: "connection",
    path: rootPath,
    title: `${params.tree.accountDisplayName} Gmail connection`,
    summary: params.tree.summary ?? `Gmail connection memory for ${params.tree.accountDisplayName}.`,
    observedAt: params.tree.updatedAt,
    metadata: {
      provider: params.tree.provider,
      account_namespace: params.tree.accountNamespace,
  ...
  if (contacts.size > 0) {
    ...
    drafts.push({
      ...
      path: contactsPath,
      title: "Contacts",
      summary: `Derived Gmail contacts for ${params.tree.accountDisplayName}.`,
      observedAt: params.tree.updatedAt,
      isMaterialized: true,
    });
...
function buildGoogleDriveSemanticMemoryTree(params: {
  ...
  drafts.push({
    ...
    nodeKind: "connection",
    path: rootPath,
    title: `${params.tree.accountDisplayName} Google Drive connection`,
    summary: params.tree.summary ?? `Google Drive connection memory for ${params.tree.accountDisplayName}.`,
    observedAt: params.tree.updatedAt,
    metadata: {
      provider: params.tree.provider,
      account_namespace: params.tree.accountNamespace,
...
function buildTwitterSemanticMemoryTree(params: {
  ...
  drafts.push({
    ...
    nodeKind: "connection",
    path: rootPath,
    title: `${params.tree.accountDisplayName} Twitter connection`,
    summary: params.tree.summary ?? `Twitter connection memory for ${params.tree.accountDisplayName}.`,
    observedAt: params.tree.updatedAt,
    metadata: {
      provider: params.tree.provider,
      account_namespace: params.tree.accountNamespace,
...
function buildGoogleCalendarSemanticMemoryTree(params: {
  ...
  drafts.push({
    ...
    nodeKind: "connection",
    path: rootPath,
    title: `${params.tree.accountDisplayName} Google Calendar connection`,
    summary: params.tree.summary ?? `Google Calendar connection memory for ${params.tree.accountDisplayName}.`,
    observedAt: params.tree.updatedAt,
    metadata: {
      provider: params.tree.provider,
      account_namespace: params.tree.accountNamespace,
...
function buildLinkedInSemanticMemoryTree(params: {
  ...
  drafts.push({
    ...
    nodeKind: "connection",
    path: rootPath,
    title: `${params.tree.accountDisplayName} LinkedIn connection`,
    summary: params.tree.summary ?? `LinkedIn connection memory for ${params.tree.accountDisplayName}.`,
    observedAt: params.tree.updatedAt,
    metadata: {
      provider: params.tree.provider,
      account_namespace: params.tree.accountNamespace,
...
function buildSlackSemanticMemoryTree(params: {
  ...
  drafts.push({
    ...
    nodeKind: "connection",
    path: rootPath,
    title: `${params.tree.accountDisplayName} Slack connection`,
    summary: params.tree.summary ?? `Slack connection memory for ${params.tree.accountDisplayName}.`,
    observedAt: params.tree.updatedAt,
    metadata: {
      provider: params.tree.provider,
      account_namespace: params.tree.accountNamespace,
  ...
  switch (params.tree.provider) {
    ...
    case "outlook":
      return buildBranchOnlyIntegrationSemanticMemoryTree({
        ...
        leaves: params.leaves,
        leafBodies: params.leafBodies,
        connectionTitle: `${params.tree.accountDisplayName} Outlook connection`,
        defaultSummary: `Outlook connection memory for ${params.tree.accountDisplayName}.`,
    ...
    case "googlesheets":
      return buildBranchOnlyIntegrationSemanticMemoryTree({
        ...
        leaves: params.leaves,
        leafBodies: params.leafBodies,
        connectionTitle: `${params.tree.accountDisplayName} Google Sheets connection`,
        defaultSummary: `Google Sheets connection memory for ${params.tree.accountDisplayName}.`,
    ...
    case "googledocs":
      return buildBranchOnlyIntegrationSemanticMemoryTree({
        ...
        leaves: params.leaves,
        leafBodies: params.leafBodies,
        connectionTitle: `${params.tree.accountDisplayName} Google Docs connection`,
        defaultSummary: `Google Docs connection memory for ${params.tree.accountDisplayName}.`,
    ...
    case "hubspot":
      return buildBranchOnlyIntegrationSemanticMemoryTree({
        ...
        leaves: params.leaves,
        leafBodies: params.leafBodies,
        connectionTitle: `${params.tree.accountDisplayName} HubSpot connection`,
        defaultSummary: `HubSpot connection memory for ${params.tree.accountDisplayName}.`,
    ...
    case "linear":
      return buildBranchOnlyIntegrationSemanticMemoryTree({
        ...
        leaves: params.leaves,
        leafBodies: params.leafBodies,
        connectionTitle: `${params.tree.accountDisplayName} Linear connection`,
        defaultSummary: `Linear connection memory for ${params.tree.accountDisplayName}.`,
    ...
    case "jira":
      return buildBranchOnlyIntegrationSemanticMemoryTree({
        ...
        leaves: params.leaves,
        leafBodies: params.leafBodies,
        connectionTitle: `${params.tree.accountDisplayName} Jira connection`,
        defaultSummary: `Jira connection memory for ${params.tree.accountDisplayName}.`,
...
function integrationTreeIdentity(params: {
  ...
  const labelSlug = safePathSegment(params.accountDisplayName || params.accountNamespace, "account");
  const accountHash = sha256(`${params.workspaceId}|${params.provider}|${params.accountNamespace}`).slice(0, 12);
  return {
...
function workspaceIntegrationTreeRecord(params: {
  workspaceId: string;
  treeId: string;
  provider: string;
  accountNamespace: string;
  ...
  slug: string;
  summary?: string | null;
  status?: IntegrationTreeRecord["status"];
  createdAt?: string;
  updatedAt?: string;
}): IntegrationTreeRecord {
  const timestamp = params.updatedAt ?? utcNowIso();
  return {
    workspaceId: params.workspaceId,
    treeId: params.treeId,
    provider: params.provider,
    ownerUserId: "",
    accountNamespace: params.accountNamespace,
    accountDisplayName: params.accountDisplayName,
    accountKey: params.accountNamespace,
    accountLabel: params.accountDisplayName,
    slug: params.slug,
...
function upsertWorkspaceIntegrationSemanticRoot(params: {
  store: RuntimeStateStore;
  tree: IntegrationTreeRecord;
}): void {
  const workspaceId = params.tree.workspaceId;
  if (!workspaceId) {
    return;
  }
  params.store.migrateSemanticMemoryTreeCategory({
  ...
  writeFileIfChanged(
    absolutePathForRelative(params.store.workspaceRoot, path),
    body,
  );
  ...
  params.store.setWorkspaceRuntimeMetadata({
    workspaceId,
    key: `integration_semantic_root_backfill_v1:${params.tree.treeId}`,
    value: "true",
    updatedAt: params.tree.updatedAt,
  });
}

function resolveWorkspaceIntegrationTree(params: {
  store: RuntimeStateStore;
  workspaceId: string;
  treeId?: string | null;
  provider?: string | null;
  ...
  const treeId = params.treeId?.trim() ?? null;
  const provider = params.provider?.trim() ?? null;
  const accountNamespace = params.accountNamespace?.trim() ?? null;
  const slug = params.slug?.trim() ?? null;
  return visibleTrees.find((tree) =>
    (treeId ? tree.treeId === treeId : true)
    && (provider ? tree.provider === provider : true)
    && (accountNamespace ? tree.accountNamespace === accountNamespace : true)
    && (slug ? tree.slug === slug : true)
  ) ?? null;
}
...
function ensureIntegrationTree(params: {
  ...
  const existing = resolveWorkspaceIntegrationTree({
    store: params.store,
    workspaceId: params.workspaceId,
  ...
  if (existing) {
    const record = workspaceIntegrationTreeRecord({
      workspaceId: params.workspaceId,
  ...
  params.store.setWorkspaceRuntimeMetadata({
    workspaceId: params.workspaceId,
    key: integrationSemanticLeafBackfillMetadataKey(record.treeId),
    value: "true",
...
function semanticStatusForIntegrationLeafStatus(
  status: IntegrationLeafRecord["status"],
...
function workspaceIntegrationLeafRecord(params: {
  workspaceId: string;
  treeId: string;
  leafId: string;
  subjectKey: string;
  ...
  const timestamp = params.updatedAt ?? params.observedAt ?? utcNowIso();
  return {
    workspaceId: params.workspaceId,
    leafId: params.leafId,
    treeId: params.treeId,
    ...
    admissionConfidence: params.admissionConfidence ?? null,
    observedAt: params.observedAt ?? null,
    supersedesLeafId: params.supersedesLeafId ?? null,
...
export function upsertWorkspaceIntegrationSemanticLeafProjection(params: {
  store: RuntimeStateStore;
  workspaceId: string;
  tree: IntegrationTreeRecord;
  leaf: IntegrationLeafRecord;
  ...
  const category: IntegrationSemanticCategory = "workspace";
  const nodeId = semanticIntegrationLeafNodeId(params.tree.treeId, params.leaf.leafId);
  const existingNode = params.store.getSemanticMemoryNode({
    category,
    workspaceId: params.workspaceId,
    treeId: params.tree.treeId,
  ...
  const defaultPath = semanticLeafRelativePath(
    semanticIntegrationTreeRelativePath(params.tree.slug),
    params.leaf,
  );
  const nodePath = existingNode?.path ?? defaultPath;
  const body = params.body
    ?? readFileIfExists(
      absolutePathForRelative(params.store.workspaceRoot, params.leaf.path),
    )
    ?? fallbackLeafBody(params.leaf);
  writeFileIfChanged(
    absolutePathForRelative(params.store.workspaceRoot, nodePath),
    body,
  );
  params.store.upsertSemanticMemoryNode({
    category,
    ...
    nodeId,
    nodeClass: "leaf",
    nodeKind: existingNode?.nodeKind ?? "leaf",
    sourceLeafId: params.leaf.leafId,
    path: nodePath,
    title: params.leaf.title,
    summary: params.leaf.summary,
    ...
    childCount: 0,
    observedAt: params.leaf.observedAt ?? params.leaf.updatedAt,
    status: semanticStatusForIntegrationLeafStatus(params.leaf.status),
    isMaterialized: existingNode?.isMaterialized ?? false,
    metadata: {
      ...(existingNode?.metadata ?? {}),
      subject_key: params.leaf.subjectKey,
  ...
  params.store.upsertSemanticMemoryEvidenceRef({
    category,
    workspaceId: params.workspaceId,
    treeId: params.tree.treeId,
    nodeId,
    refId: params.leaf.leafId,
    provider: params.tree.provider,
    accountNamespace: params.tree.accountNamespace,
    connectionId: null,
    externalObjectId: params.leaf.externalObjectId ?? null,
    externalObjectType: params.leaf.externalObjectType ?? null,
    sourceType: params.leaf.sourceType ?? null,
    sourceEventId: params.leaf.sourceEventId ?? null,
    sourceMessageId: params.leaf.sourceMessageId ?? null,
    ...
    metadata: {
      ...
      title: params.leaf.title,
      subject_key: params.leaf.subjectKey,
      entity_key: params.leaf.entityKey ?? null,
      entity_label: params.leaf.entityLabel ?? null,
      branch_key: params.leaf.branchKey ?? null,
      branch_label: params.leaf.branchLabel ?? null,
      fingerprint: params.leaf.fingerprint,
      body_sha256: params.leaf.bodySha256,
      tags: params.leaf.tags,
    },
  ...
  params.store.setWorkspaceRuntimeMetadata({
    workspaceId: params.workspaceId,
    key: integrationSemanticLeafBackfillMetadataKey(params.tree.treeId),
    value: "true",
    updatedAt: params.leaf.updatedAt,
  });
}
...
function integrationTreeBody(params: {
  ...
  const lines = [
    `# ${params.tree.accountDisplayName}`,
    "",
    `- Tree ID: \`${params.tree.treeId}\``,
    `- Workspace: ${params.tree.workspaceId ?? "unknown"}`,
    `- Provider: ${params.tree.provider}`,
    `- Account namespace: ${params.tree.accountNamespace}`,
    `- Active leaves: ${params.leafCount}`,
    `- Active summaries: ${params.summaryCount}`,
    "",
    "## Summary",
    "",
    params.tree.summary ?? `${params.tree.accountDisplayName} integration memory tree.`,
    "",
  ];
  return `${lines.join("\n").trim()}\n`;
}
...
function integrationEntityBody(params: {
  ...
  const lines = [
    `# ${params.entityLabel}`,
    "",
    `- Tree: ${params.tree.accountDisplayName}`,
    `- Provider: ${params.tree.provider}`,
    ...
    "## Summary",
    "",
    `${params.entityLabel} in ${params.tree.accountDisplayName}.`,
    "",
  ];
  return `${lines.join("\n").trim()}\n`;
}
...
function integrationBranchBody(params: {
  ...
  const lines = [
    ...
    "## Summary",
    "",
    `${params.branchLabel} in ${params.tree.accountDisplayName}.`,
    "",
  ].filter((line): line is string => typeof line === "string");
  return `${lines.join("\n").trim()}\n`;
}
...
function semanticEvidenceRefsForIntegrationTree(params: {
  tree: IntegrationTreeRecord;
  semantic: SemanticIntegrationTreeBuildResult;
  leaves: IntegrationLeafRecord[];
}): SemanticIntegrationEvidenceRef[] {
  const leafById = new Map(
    params.leaves.map((leaf) => [leaf.leafId, leaf] as const),
  );
  const refs: SemanticIntegrationEvidenceRef[] = [];
  for (const node of params.semantic.nodes) {
    ...
    refs.push({
      ...
      metadata: {
        ...
        entity_key: leaf.entityKey ?? null,
        entity_label: leaf.entityLabel ?? null,
        branch_key: leaf.branchKey ?? null,
        branch_label: leaf.branchLabel ?? null,
...
function buildIntegrationCanonicalNodesAndEdges(params: {
  ...
  nodes.set(integrationTreeNodeId(params.tree.treeId), {
    ...
    nodeKind: "tree",
    path: treePath,
    title: params.tree.accountDisplayName,
    summary: params.tree.summary ?? `${params.tree.accountDisplayName} integration memory tree.`,
  ...
  for (const [entityKey, stat] of entityStats) {
    ...
    nodes.set(integrationEntityNodeId(params.tree.treeId, entityKey), {
      ...
      path: entityPath,
      title: stat.label,
      summary: `${stat.label} in ${params.tree.accountDisplayName}.`,
  ...
  for (const scope of scopeGroups.values()) {
    ...
    nodes.set(integrationBranchNodeId(params.tree.treeId, scope.entityKey ?? null, scope.branchKey), {
      ...
      summary: `${scope.branchLabel ?? scope.branchKey} in ${params.tree.accountDisplayName}.`,
  ...
  if (contactRelations.length > 0) {
    const contactBranchBody = [
      ...
      "## Summary",
      "",
      `Derived contacts for ${params.tree.accountDisplayName}.`,
      "",
    ].join("\n");
    nodes.set(integrationBranchNodeId(params.tree.treeId, null, "contacts"), {
      ...
      title: "Contacts",
      summary: `Derived contacts for ${params.tree.accountDisplayName}.`,
...
async function buildSummaryTreePlan(params: {
  ...
  for (const entityGroup of entityGroups.values()) {
    ...
    for (const branchGroup of branchGroups.values()) {
      ...
      const branchSubtree = await buildBranchSummarySubtree({
        tree: params.tree,
        title: branchTitle,
        scopeLabel: `${params.tree.accountDisplayName} ${branchTitle}`.trim(),
    ...
    if (entityGroup.entityKey) {
      ...
      const entityNode = await buildTempSummaryNode({
        tree: params.tree,
        title: entityTitle,
        scopeLabel: `${params.tree.accountDisplayName} ${entityTitle}`.trim(),
    ...
    const accountNode = await buildTempSummaryNode({
      tree: params.tree,
      title: `${params.tree.accountDisplayName} account context`,
      scopeLabel: `${params.tree.accountDisplayName} account context`,
      children: branchRoots.map((node) => summaryNodeAsChild(node)),
      ordinal: rootChildren.length + 1,
      modelClient: params.modelClient,
    });
  ...
  if (rootChildren.length === 1) {
    ...
    const rootNode = await buildTempSummaryNode({
      tree: params.tree,
      title: `${params.tree.accountDisplayName} integration memory`,
      scopeLabel: `${params.tree.accountDisplayName} integration memory`,
...
async function syncNodeEmbedding(params: {
  ...
  const embeddingText = buildEmbeddingText({
    treeLabel: params.tree.accountDisplayName,
    provider: params.tree.provider,
    title: params.title,
    summary: params.summary,
    excerpt,
  ...
  // workspace-removal Piece 5.7: integration embeddings are control-plane-only; the
  // store ignores any workspaceId (and tree.workspaceId is null now), so it is not
  // threaded.
...
export async function persistIntegrationCandidate(params: {
  ...
  const tree = ensureIntegrationTree({
    ...
    accountNamespace: params.candidate.accountNamespace,
    accountDisplayName: params.candidate.accountDisplayName,
  });
  ...
  const activeLeaves = listActiveIntegrationLeafView({
    store: params.store,
    workspaceId: params.workspaceId,
    tree,
  });
  const existingDuplicate = activeLeaves.find((leaf) => leaf.fingerprint === contentFingerprint) ?? null;
  if (existingDuplicate) {
    ...
    if (needsMetadataRefresh) {
      ...
      const refreshed = workspaceIntegrationLeafRecord({
        workspaceId: params.workspaceId,
        ...
        updatedAt: params.candidate.observedAt ?? utcNowIso(),
      });
      upsertWorkspaceIntegrationSemanticLeafProjection({
        store: params.store,
        workspaceId: params.workspaceId,
  ...
  const existingActive = activeLeaves
    .filter((leaf) => leaf.subjectKey === params.candidate.subjectKey && leaf.status === "active")
    .sort((left, right) =>
      (right.observedAt ?? right.updatedAt).localeCompare(left.observedAt ?? left.updatedAt)
      || right.updatedAt.localeCompare(left.updatedAt)
      || right.createdAt.localeCompare(left.createdAt)
      || right.leafId.localeCompare(left.leafId),
    )[0] ?? null;
  const absolutePath = absolutePathForRelative(params.store.workspaceRoot, relativePath);
  writeFileIfChanged(absolutePath, params.candidate.content);
  ...
  if (existingActive && existingActive.fingerprint !== contentFingerprint) {
    const supersededAt = params.candidate.observedAt ?? utcNowIso();
    const supersededLeaf = workspaceIntegrationLeafRecord({
      workspaceId: params.workspaceId,
      leafId: existingActive.leafId,
      treeId: tree.treeId,
      subjectKey: existingActive.subjectKey,
      entityKey: existingActive.entityKey ?? null,
      entityLabel: existingActive.entityLabel ?? null,
      branchKey: existingActive.branchKey ?? null,
      branchLabel: existingActive.branchLabel ?? null,
      path: existingActive.path,
      title: existingActive.title,
      summary: existingActive.summary,
      fingerprint: existingActive.fingerprint,
      bodySha256: existingActive.bodySha256,
      tags: existingActive.tags,
      sourceType: existingActive.sourceType ?? null,
      sourceEventId: existingActive.sourceEventId ?? null,
      sourceMessageId: existingActive.sourceMessageId ?? null,
      externalObjectId: existingActive.externalObjectId ?? null,
      externalObjectType: existingActive.externalObjectType ?? null,
      admissionConfidence: existingActive.admissionConfidence ?? null,
      observedAt: existingActive.observedAt ?? null,
      supersedesLeafId: existingActive.supersedesLeafId ?? null,
      supersededAt,
      status: "superseded",
    ...
    params.store.updateSemanticMemoryLeafStatusBySourceLeafIds({
      category: ACTIVE_WORKSPACE_INTEGRATION_CATEGORY,
      workspaceId: params.workspaceId,
      treeId: tree.treeId,
      sourceLeafIds: [existingActive.leafId],
      status: "superseded",
      updatedAt: supersededAt,
    });
    outcome = "superseding";
  ...
  const leaf = workspaceIntegrationLeafRecord({
    workspaceId: params.workspaceId,
    leafId,
    treeId: tree.treeId,
    subjectKey: params.candidate.subjectKey,
...
export async function rebuildIntegrationTree(params: {
  ...
  const tree = params.tree
    ?? resolveWorkspaceIntegrationTree({
      store: params.store,
      workspaceId,
      treeId: params.treeId,
    });
  ...
  const semanticCategory = writableIntegrationSemanticCategory(workspaceId);
  const activeLeaves = listActiveIntegrationLeafView({
    store: params.store,
    workspaceId,
    tree,
  ...
  for (const leaf of activeLeaves) {
    ...
    if (leaf.path !== relativePath) {
      rewrittenLeaves.push(
        workspaceIntegrationLeafRecord({
          workspaceId,
          leafId: leaf.leafId,
          treeId: leaf.treeId,
          subjectKey: leaf.subjectKey,
  ...
  if (semantic) {
    ...
    fs.rmSync(legacySemanticIntegrationTreeDir(params.store.workspaceRoot, tree.slug), {
      recursive: true,
      force: true,
    });
    ...
    params.store.replaceSemanticMemoryEvidenceRefs({
      ...
      workspaceId,
      treeId: params.treeId,
      refs: semanticEvidenceRefsForIntegrationTree({
        tree,
        semantic,
        leaves: rewrittenLeaves,
      }),
...
export function accessibleIntegrationTreesForWorkspace(params: {
  store: RuntimeStateStore;
  workspaceId: string;
  treeId?: string | null;
}): IntegrationTreeRecord[] {
...
function nodeScore(params: {
  ...
  let score = textScore(
    params.query,
    params.candidate.tree.accountNamespace,
...
function candidateToHit(params: {
  ...
  return {
    ...
    account_namespace: params.candidate.tree.accountNamespace,
...
export async function retrieveIntegrationMemory(params: {
  ...
  const categoryCache = new Map<string, IntegrationSemanticCategory>();
  const trees = accessibleIntegrationTreesForWorkspace({
    store: params.store,
    workspaceId: params.workspaceId,
    treeId: params.treeId ?? null,
  ...
  return {
    entries: result.hits.map((hit) => ({
      ...
      freshness_note: hit.node_kind === "leaf"
        ? `Leaf memory from ${hit.provider} account ${hit.account_namespace}.`
        : `Structured memory node from ${hit.provider} account ${hit.account_namespace}.`,
