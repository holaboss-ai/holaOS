...
export interface BuildAgentCapabilityManifestParams {
  ...
  composioInlineToolRefs?: ComposioInlineToolCapabilityRef[] | null;
...
function buildComposioInlineDescriptor(
  ref: ComposioInlineToolCapabilityRef,
): StaticAgentCapabilityDescriptor {
  const executionMode: AgentCapabilityExecutionMode = "tool_call";
  return {
    id: ref.name,
    kind: "mcp_tool",
    policy: ref.read_only ? "inspect" : "mutate",
    title: titleFromToken(ref.name),
    description: `Composio integration '${ref.toolkit_slug}' tool — connected for this run.`,
    source: "composio_inline",
    callable_spec: {
      kind: "fixed",
      callable_name: ref.name,
    },
    visibility_surface: "tool",
    permission_surface: "mcp_tool",
    execution_mode: executionMode,
...
export interface ComposioInlineToolCapabilityRef {
  name: string;
  toolkit_slug: string;
  tool_slug: string;
...
function buildStaticCapabilityRegistry(
  ...
  for (const inlineRef of params.composioInlineToolRefs ?? []) {
    upsertDescriptor(buildComposioInlineDescriptor(inlineRef));
  }
...
export function renderCapabilityPolicyCorePromptSection(
  ...
  if (normalizedSessionKind === "main_session") {
    lines.push(
      ...
      "When inspect or verification capabilities are surfaced, use them to confirm state-changing work before claiming success.",
    );
    return lines.join("\n");
  }
  lines.push(
...
export function renderCapabilityToolRoutingPromptSection(
  ...
  const hasRemember = manifest.runtime_tools.some(
    (capability) => capability.id === "remember",
  );
  ...
  if (hasBashTool && hasDirectWorkspaceFileTool) {
    ...
    lines.push("User-facing artifacts: when you produce a file FOR THE USER (a screenshot, generated document, export, chart, etc.), save it under the workspace `outputs/` directory (e.g. `outputs/screenshots/<name>.png`) — NOT `~/Downloads` or other host paths. Files under `outputs/` are registered as viewable artifacts the user can open and that are delivered across channels (desktop card, IM attachment); a bare host path is not. Use a descriptive filename.");
  }
  if (manifest.runtime_tools.some((capability) => capability.id === "cronjobs_create")) {
    ensureHeading();
    lines.push("Cronjob routing: cronjobs (surfaced as \"Automations\" in the UI) are the workspace-level scheduler. A cronjob = one cron expression + one instruction string that fires a subagent on schedule.");
    lines.push("When the user asks for a recurring task on a schedule — \"every morning at 9\", \"daily\", \"hourly\", \"each Monday\", \"every 5 minutes\", \"at 17:00 on weekdays\", \"twice a week\" — use `cronjobs_create`. A step like \"when the cron fires, generate a digest, save it, and post a notification\" can be one cronjob whose instruction describes those steps to the spawned subagent.");
    lines.push("Manage existing schedules with `cronjobs_list`, `cronjobs_get`, `cronjobs_update`, `cronjobs_delete`, and `cronjobs_run_now`. Pause a schedule with `cronjobs_update` setting `enabled: false` instead of deleting it; the user can usually restart it later.");
    lines.push("When creating a cronjob, set `description` to the user-facing summary and `instruction` to the prompt the spawned subagent will execute each fire. If you only have one phrasing, pass it as both. Times in the cron expression resolve in the workspace user's timezone.");
  }
  if (manifest.runtime_tools.some((capability) => capability.id === "holaboss_workspace_integrations_propose_connect")) {
    ...
    lines.push("Browser fallback for a third-party service is appropriate only when the user explicitly asks for a browser-based action, when the provider has no integration in the workspace catalog, or — for a READ-ONLY view of a specific page — when the integration is already connected but that exact page genuinely cannot be reached through its surfaced tools.");
    lines.push("Publish/send/create actions MUST go through the integration. When a service's integration is required by the active capability or already connected, and its `<toolkit>_<verb>` tools cover the requested publish, post, send, or create action, complete it with those tools. NEVER ask the user to log into the embedded browser (or any browser) to publish/post/send manually as a workaround. If the specific posting tool is not surfaced this run, call `holaboss_workspace_integrations_propose_connect` for that toolkit (or state the precise missing tool) — do NOT substitute a manual browser login.");
  ...
  if (manifest.runtime_tools.some((capability) => capability.id === "delegate_task")) {
    ensureHeading();
    lines.push("Delegation routing: when the user asks for work that needs capability genuinely missing from this run, use `delegate_task` instead of replying that the current run lacks those tools.");
    lines.push("Deliverable routing: for long, multi-source, evidence-heavy deliverables (full reports, briefs, digests with many sections), `delegate_task` can produce the result as an artifact while the main chat stays responsive. For shorter or focused write-ups, produce them directly.");
    lines.push("Use `delegate_task` for work that is genuinely long-running, multi-step, parallel, or interruptible. Direct execution is the right route for small to medium work that fits a single turn — `delegate_task` is for parallelism and durability, not for offloading every execution.");
    ...
    lines.push("Do not lead with a capability apology, manual workaround, or \"I can't do that here\" answer when delegation or another direct route is available.");
  ...
  if (manifest.runtime_tools.some((capability) => capability.id === "open_macos_settings")) {
    ensureHeading();
    lines.push("macOS permissions: when a host operation fails because the macOS host lacks a privacy permission (e.g. `screencapture` reporting `could not create image from display` → Screen Recording; UI scripting → Accessibility; protected files → Full Disk Access), do NOT give up. Call `open_macos_settings` with the matching pane — it registers Holaboss with macOS where the OS supports a prompt (so it appears in the relevant Settings list) and opens that pane. Tell the user to enable Holaboss there, then retry the original operation.");
  }
  ...
  if (hasRemember) {
    ensureHeading();
    lines.push(
      "Memory capture: when `remember` is surfaced, call it the moment you learn something durable that future sessions should reuse — a stable user fact or preference, a project decision or constraint, a verified procedure, a recurring blocker, or a reference. Record it as you learn it, in the same turn; do not wait until the end or assume it will be captured automatically.",
    );
    lines.push(
      "Give `remember` a specific, self-contained `summary` (the fact itself, not `the user asked about X`). Use `scope: 'user'` for stable facts about the person and `workspace` for project knowledge; reuse a stable `subject_key` when updating something you recorded before. Do NOT record one-off task requests, unresolved hypotheses, transient runtime state, or anything already in the current context.",
    );
  }
  if (
...
export function renderCapabilityAvailabilityContextPromptSection(
  ...
  if (
    ...
    lines.push(
      "This session is a full-capability assistant. Treat the surfaced tools above as your direct authority and execute the work that fits this turn; reach for `delegate_task` when the work is genuinely long-running, multi-step, parallel, or interruptible.",
    );
  }
  return lines.join("\n");
}
