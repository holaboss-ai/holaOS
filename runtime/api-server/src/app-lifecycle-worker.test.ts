...
test("isReapableOwnOrphan only reaps a listener whose cwd is this app's dir", () => {
  const appDir = path.join(os.tmpdir(), "ws", "apps", "github_tracker");

  // Same dir, or a subdir the app chdir'd into → our orphan, reapable.
  assert.equal(isReapableOwnOrphan({ listenerCwd: appDir, appDir }), true);
  assert.equal(
    isReapableOwnOrphan({ listenerCwd: path.join(appDir, "dist"), appDir }),
    true,
  );

  // Unknown cwd → never reap (can't prove it's ours).
  assert.equal(isReapableOwnOrphan({ listenerCwd: null, appDir }), false);

  // A different app holding the port → never reap.
  const otherApp = path.join(os.tmpdir(), "ws", "apps", "linkedin");
  assert.equal(isReapableOwnOrphan({ listenerCwd: otherApp, appDir }), false);

  // Parent dir → never reap.
  assert.equal(
    isReapableOwnOrphan({ listenerCwd: path.dirname(appDir), appDir }),
    false,
  );

  // Sibling whose name is a string-prefix of ours → never reap (guards against
  // a naive startsWith without a path separator).
  assert.equal(
    isReapableOwnOrphan({ listenerCwd: `${appDir}_evil`, appDir }),
    false,
  );
});
...
test("runtime executor resolves store-backed integration env for bound shell apps", async () => {
  ...
  // Single-tenant: the runtime hosts one canonical "root" workspace. The
  // executor infers the workspace from the appDir by matching it against
  // store.workspaceDir(id) for each listWorkspaces() entry — which is only the
  // root — so the app must live under the root workspace dir and its integration
  // binding must be registered under root for the env to resolve.
...
test("runtime executor uses explicit workspace id for integration env when app dir cannot be inferred", async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "hb-shell-app-explicit-workspace-"));
  const workspaceRoot = path.join(root, "workspace");
  const store = new RuntimeStateStore({
    dbPath: path.join(root, "runtime.db"),
    workspaceRoot
  ...
  const appDir = fs.mkdtempSync(path.join(os.tmpdir(), "hb-shell-app-outside-workspace-"));
  store.upsertIntegrationConnection({
    connectionId: "conn-twitter-1",
    providerId: "twitter",
    ownerUserId: "user-1",
    accountLabel: "HolaUser",
    authMode: "composio",
    grantedScopes: [],
    status: "active",
  ...
  store.upsertIntegrationBinding({
    bindingId: "bind-twitter-default",
    workspaceId: workspace.id,
    targetType: "workspace",
    targetId: "default",
  ...
  await executor.startApp({
    ...
    resolvedApp: {
      ...
      mcpTools: [],
      healthCheck: { path: "/health", timeoutS: 1, intervalS: 0.01 },
      envContract: ["WORKSPACE_TWITTER_INTEGRATION_ID", "WORKSPACE_API_URL"],
      integrations: [
        {
          key: "twitter",
          provider: "twitter",
          capability: "social.metrics",
          scopes: [],
          required: true,
          credentialSource: "platform",
          holabossUserIdRequired: true,
      ...
      startCommand: "",
      baseDir: "apps/x-engagement-tracker",
      lifecycle: { setup: "", start: "npm run start", stop: "npm run stop" }
    },
    httpPort: 18081,
    mcpPort: 13101,
