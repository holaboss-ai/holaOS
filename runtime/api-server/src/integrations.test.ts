...
test("rejects missing connections and cross-provider bindings; unknown workspace ids resolve to the root", () => {
  const root = makeTempDir("hb-integrations-");
  const store = new RuntimeStateStore({
    dbPath: path.join(root, "runtime.db"),
    workspaceRoot: path.join(root, "workspace")
  ...
  // Single-tenant (workspace-removal): store.getWorkspace() resolves ANY id to
  // the synthetic root, so requireWorkspace no longer rejects an unknown id.
  // listBindings now resolves to the root and returns the (empty) binding set
  // for that id rather than throwing "workspace not found".
  assert.deepEqual(service.listBindings({ workspaceId: "missing-workspace" }), {
    bindings: []
  });
...
test("createConnection dedupes a re-auth flow on the same handle: keeps connection_id, refreshes external_id and secret_ref, preserves bindings", () => {
  ...
  // Identity preserved.
  assert.equal(second.account_handle, "alice");
  // Only one row exists.
...
test("updateConnection backfills identity onto a legacy NULL-identity row without clobbering other fields", () => {
  ...
  assert.equal(backfilled.account_handle, "alice");
  assert.equal(backfilled.account_email, "j@example.com");
  // Other fields preserved.
  assert.equal(backfilled.account_label, "@alice");
