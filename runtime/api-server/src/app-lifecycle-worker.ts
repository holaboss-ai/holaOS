...
/** Pure decision for the pre-start reaper: is a process listening on one of
 *  this app's ports a reapable orphan of THIS SAME app? True ONLY when the
 *  listener's cwd resolves to (or inside) appDir. A null/unknown cwd, or a cwd
 *  belonging to another app, returns false — we never kill a process we can't
 *  prove is our own orphan, so a different app holding a port is left intact. */
export function isReapableOwnOrphan(params: {
  listenerCwd: string | null;
  appDir: string;
}): boolean {
  if (!params.listenerCwd) {
    return false;
  }
  const expected = path.resolve(params.appDir);
  const observed = path.resolve(params.listenerCwd);
  return observed === expected || observed.startsWith(`${expected}${path.sep}`);
}

/** Before (re)spawning an app, free its allocated ports by reaping ONLY an
 *  orphaned listener that is provably this same app (cwd inside appDir).
 *  Foreign listeners — another app, or a process whose cwd we can't read — are
 *  left untouched: the subsequent bind fails loudly rather than us taking down
 *  someone else's server.
 *
 *  This is the common failure after a runtime restart: the previous app child
 *  is reparented to init and keeps squatting on the HTTP/MCP port, so every
 *  fresh spawn dies with EADDRINUSE before it can become healthy. Only runs on
 *  the unhealthy path — a healthy instance short-circuits before we get here. */
async function reapOwnOrphanListeners(params: {
  appId: string;
  appDir: string;
  httpPort: number;
  mcpPort: number;
  ...
  for (const port of [params.httpPort, params.mcpPort]) {
    const pid = lookupListenerPid(port);
    if (pid === null) {
      continue;
    }
    const listenerCwd = lookupProcessCwd(pid);
    if (!isReapableOwnOrphan({ listenerCwd, appDir: params.appDir })) {
      writeAppLifecycleEvent(params.appDir, {
        event: "app.reap_orphan.skip_foreign",
        app_id: params.appId,
        port,
        pid,
        listener_cwd: listenerCwd,
      });
      continue;
    }
    writeAppLifecycleEvent(params.appDir, {
      event: "app.reap_orphan.kill",
      app_id: params.appId,
      port,
      pid,
    });
    ...
    // Give it up to ~2s to release the socket, then SIGKILL if the same pid
    // still holds it.
    const deadlineMs = Date.now() + 2000;
    while (Date.now() < deadlineMs && lookupListenerPid(port) === pid) {
      await new Promise((resolve) => setTimeout(resolve, 100));
    }
    if (lookupListenerPid(port) === pid) {
      try {
...
export async function startShellLifecycleAppTarget(params: {
  ...
  return await withAppStartOperation(params, async () => {
    ...
    // Free our own ports first: a prior instance of THIS app, orphaned by a
    // runtime restart, can still be squatting on the HTTP/MCP port and would
    // otherwise make the fresh spawn die with EADDRINUSE before it can become
    // healthy. Foreign processes are never touched.
    await reapOwnOrphanListeners({
      appId: params.appId,
      appDir: params.appDir,
  ...
  async startApp(params: AppLifecycleStartParams): Promise<AppLifecycleActionResult> {
    ...
    const integrationRuntime =
      this.store && params.appDir && params.resolvedApp
        ? await resolveIntegrationRuntime({
            store: this.store,
            appId: params.appId,
