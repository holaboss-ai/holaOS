...
export class FilesystemMemoryService implements MemoryServiceLike {
  ...
  async status(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    if (this.#store) {
      ...
      const graph = summarizeWorkspaceMemoryGraph({
        store: this.#store,
        workspaceId,
      });
      status.provider = "workspace_graph";
      status.custom = {
        ...(status.custom as Record<string, unknown>),
        workspace_graph_roots: graph.roots,
        workspace_graph_leaves: graph.leaves,
        workspace_graph_semantic_nodes: graph.semanticNodes,
        workspace_graph_semantic_internal_nodes: graph.semanticInternalNodes,
        workspace_graph_semantic_edges: graph.semanticEdges,
        workspace_graph_semantic_relations: graph.semanticRelations,
      };
    }
    return status;
  }
  ...
  async sync(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    if (this.#store) {
      const workspace = await rebuildWorkspaceMemoryGraph({
        store: this.#store,
        workspaceId,
      });
      rebuilt = {
