...
test("buildAgentCapabilityManifest classifies tools, skills, and MCP aliases", () => {
  const manifest = buildAgentCapabilityManifest({
    ...
    runtimeToolIds: ["image_generate", "todoread", "todowrite"],
    workspaceCommandIds: ["hello"],
    defaultTools: ["read", "edit", "todoread", "todowrite"],
    extraTools: ["browser_get_state", "image_generate", "todoread", "todowrite"],
  ...
  assert.deepEqual(manifest.context, {
    ...
    runtime_tool_ids: ["image_generate", "todoread", "todowrite"],
  ...
  assert.deepEqual(
    ...
    ["image_generate", "todoread", "todowrite"]
  );
  ...
  assert.ok(
    manifest.mutate.some((capability) => capability.callable_name === "image_generate")
  );
...
test("buildAgentCapabilityManifest filters browser tools when policy context does not allow them", () => {
  const manifest = buildAgentCapabilityManifest({
    ...
    browserToolIds: [],
    runtimeToolIds: ["image_generate"],
    defaultTools: ["read"],
    extraTools: ["browser_get_state", "image_generate"],
    workspaceSkillIds: [],
    resolvedMcpToolRefs: [],
  });
  ...
  assert.deepEqual(manifest.context, {
    ...
    browser_tool_ids: [],
    runtime_tool_ids: ["image_generate"],
...
test("renderCapabilityToolRoutingPromptSection emits routing addenda without coordinator-first framing", () => {
  const manifest = buildAgentCapabilityManifest({
    harnessId: "pi",
    sessionKind: "main_session",
    browserToolsAvailable: false,
  ...
  assert.match(section, /Deliverable routing:/);
  assert.match(section, /long, multi-source, evidence-heavy deliverables/i);
  assert.match(section, /genuinely long-running, multi-step, parallel, or interruptible/i);
  assert.doesNotMatch(section, /Treat the main session as a coordinator first/i);
  ...
  assert.match(section, /Do not lead with a capability apology, manual workaround, or "I can't do that here" answer when delegation or another direct route is available/i);
...
test("composioInlineToolRefs register as mcp_tool kind with composio_inline source", () => {
  const manifest = buildAgentCapabilityManifest({
    harnessId: "pi",
    sessionKind: "subagent",
    browserToolsAvailable: false,
  ...
  const notion = manifest.mcp_tools.find((entry) => entry.id === "notion_fetch_data");
  const gmail = manifest.mcp_tools.find((entry) => entry.id === "gmail_send_email");
  assert.ok(notion, "notion inline tool registered as mcp_tool");
  assert.ok(gmail, "gmail inline tool registered as mcp_tool");
  assert.equal(notion!.source, "composio_inline");
  assert.equal(gmail!.source, "composio_inline");
  assert.equal(notion!.policy, "inspect");
  assert.equal(gmail!.policy, "mutate");
  const enabled = buildEnabledToolMapFromManifest(manifest);
  assert.equal(enabled.notion_fetch_data, true);
  assert.equal(enabled.gmail_send_email, true);
});
...
test("renderDelegatedCapabilityAvailabilityContextPromptSection exposes backstage tools without expanding direct authority", () => {
  ...
  assert.match(section, /Web Search \(`web_search`\)/);
...
test("evaluateAgentCapabilities includes richer execution and authority metadata", () => {
  ...
  const runtimeCapability = evaluation.capabilities.find(
    (capability) => capability.id === "image_generate",
  );
...
test("renderCapabilityPolicyPromptSection summarizes grouped capabilities", () => {
  ...
  assert.match(section, /Coordination tools: available \(2 enabled\)\./);
  ...
  assert.match(section, /If a capability is not surfaced in the runtime context for this run, do not assume it is available\./);
  assert.match(section, /When inspect or verification capabilities are surfaced, use them to confirm state-changing work before claiming success\./);
  assert.match(section, /Connected MCP access: available\./);
  assert.match(
    section,
...
test("renderCapabilityPolicyPromptSection surfaces full-capability semantics for main_session", () => {
  const manifest = buildAgentCapabilityManifest({
    harnessId: "pi",
    sessionKind: "main_session",
    browserToolsAvailable: false,
  ...
  assert.match(section, /Browser tools: none\./);
  assert.match(section, /This session is a full-capability assistant\./);
  assert.match(section, /Treat the surfaced tools above as your direct authority and execute the work that fits this turn/i);
  assert.match(section, /reach for `delegate_task` when the work is genuinely long-running, multi-step, parallel, or interruptible/i);
});
