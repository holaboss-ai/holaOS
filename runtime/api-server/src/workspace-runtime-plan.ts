...
/**
 * Parse one MCP server map (`mcp_registry.servers` or `mcp_registry.app_servers`)
 * into `parsed`. Both sections share an identical entry shape; the runtime folds
 * them into a single compiled server set so the agent gets every tool regardless
 * of whether an MCP is standalone (user-added, `servers`) or owned by an app
 * container (`app_servers`). `section` scopes error paths to the right key.
 */
function parseServerMap(
  rawServers: unknown,
  section: "servers" | "app_servers",
  parsed: Record<string, ServerConfig>
): void {
  ...
  if (!isRecord(rawServers)) {
    err({
      code: "workspace_mcp_registry_missing",
      path: `mcp_registry.${section}`,
      message: `field 'mcp_registry.${section}' must be an object`
    });
  }
  for (const [serverId, value] of Object.entries(rawServers)) {
    ...
    const base = `mcp_registry.${section}.${serverKey}`;
    if (!isRecord(value)) {
      err({
        code: "workspace_mcp_server_unknown",
        path: base,
    ...
    const timeoutMs = parseTimeoutMs(value.timeout_ms, `${base}.timeout_ms`);
    const headers = parseStringPairs(value.headers, `${base}.headers`);
    const environment = parseStringPairs(value.environment, `${base}.environment`);

    if (serverType === "local") {
      const command =
        serverKey === WORKSPACE_SERVER_ID && (value.command === undefined || value.command === null)
          ? []
          : parseCommand(value.command, `${base}.command`);
      parsed[serverKey] = {
        server_id: serverKey,
        type: "local",
        command,
...
function readServers(registry: JsonRecord): Record<string, ServerConfig> {
  ...
  // Standalone pool first, then app-container-owned servers folded in on top.
  parseServerMap(registry.servers, "servers", parsed);
  parseServerMap(registry.app_servers, "app_servers", parsed);
  return parsed;
}
