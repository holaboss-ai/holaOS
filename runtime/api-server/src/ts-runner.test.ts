...
test("resolveTsRunnerBootstrapState resumes a CLI harness from its captured binding id but not the turn-1 placeholder", () => {
  const sandboxRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "hb-ts-runner-cli-resume-"),
  );
  process.env.HB_SANDBOX_ROOT = sandboxRoot;
  fs.mkdirSync(path.join(sandboxRoot, "workspace", "workspace-1"), {
    recursive: true,
  });

  // Turn 2+: the per-session binding holds Claude Code's own captured session
  // id (distinct from the runtime session id) → resume from it so the chat
  // keeps its context. CLI harnesses read persisted_harness_session_id.
  const resumed = resolveTsRunnerBootstrapState({
    workspace_id: "workspace-1",
    session_id: "session-1",
    input_id: "input-1",
    instruction: "hello",
    context: {
      _sandbox_runtime_exec_v1: {
        harness: "claude-code",
        harness_session_id: "claude-real-session-id",
      },
    },
    model: null,
    debug: false,
  });
  assert.equal(resumed.harness, "claude-code");
  assert.equal(resumed.persistedHarnessSessionId, "claude-real-session-id");

  // Turn 1: the binding is still the runtime session-id placeholder
  // (=== session_id), which the CLI won't recognize → start fresh, no resume.
  const fresh = resolveTsRunnerBootstrapState({
    workspace_id: "workspace-1",
    session_id: "session-1",
    input_id: "input-1",
    instruction: "hello",
  ...
  assert.equal(fresh.persistedHarnessSessionId, null);
});
...
test("runTsRunnerCli accepts the request via --request-file", async () => {
  // The runner (runner-worker) hands large requests off by file instead of on
  // the command line, so ts-runner must decode --request-file identically to
  // --request-base64. Windows caps a command line at ~32,767 chars, so inlining
  // a big request there fails the spawn with ENAMETOOLONG.
  const sandboxRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "hb-ts-runner-file-"),
  );
  process.env.HB_SANDBOX_ROOT = sandboxRoot;
  const requestFilePath = path.join(sandboxRoot, "request.b64");
  fs.writeFileSync(
    requestFilePath,
    encodeRequest({
      ...baseRequest(),
  ...
  const lines = stdout
    ...
    .map((line) => JSON.parse(line));
  assert.equal(lines[0].event_type, "run_claimed");
  assert.equal(lines.at(-1).event_type, "run_completed");
});

test("runTsRunnerCli reports a missing --request-file path", async () => {
  let stderr = "";
  const exitCode = await runTsRunnerCli(["--request-file"], {
    io: {
      stdout: {
        write() {
          return true;
  ...
  assert.match(stderr, /--request-file requires a path/);
});
...
test("runTsRunnerCli gives main workspace sessions full execution-surface tooling", async () => {
...
test("runTsRunnerCli hides the subagent delegation tools from main workspace sessions", async () => {
  setTempSandboxRoot("hb-ts-runner-hidden-delegation-");
  let capturedProjectRequest: AgentRuntimeConfigCliRequest | null = null;
  ...
  assert.equal(exitCode, 0);
  assert.ok(capturedProjectRequest);
  // The delegation family is hidden (see HIDDEN_RUNTIME_TOOL_IDS in
  // ts-runner.ts): only the non-hidden tool survives into the main session's
  // advertised runtime + extra tool sets, so the agent can neither invoke nor
  // see `delegate_task` and its task-management siblings.
  assert.deepEqual(
    (capturedProjectRequest as { runtime_tool_ids: string[] }).runtime_tool_ids,
    ["web_search"],
  );
  ...
  const extraTools = (capturedProjectRequest as { extra_tools: string[] })
    .extra_tools;
  for (const hidden of hiddenIds) {
    assert.ok(
      !extraTools.includes(hidden),
      `expected ${hidden} to be hidden from the main session`,
    );
  }
});
...
test("runTsRunnerCli exposes direct MCP tools to main-session requests and forwards them to delegated subagents", async () => {
  const sandboxRoot = fs.mkdtempSync(
...
test("runTsRunnerCli derives recalled durable memory from interaction trees", async () => {
  ...
  assert.equal(deployLeaf.category, "workspace");
  ...
  assert.ok(styleLeaf);
  assert.equal(styleLeaf.category, "workspace");
...
test("runTsRunnerCli resolves an active user-owned browser surface to the agent space (subagent)", async () => {
  let capturedHarnessRequest: Record<string, unknown> | null = null;
...
test("runTsRunnerCli resolves an active user-owned browser surface to the agent space (workspace session)", async () => {
  let capturedHarnessRequest: Record<string, unknown> | null = null;
...
test("runTsRunnerCli stages browser tools for subagent executor sessions and strips orchestration runtime tools", async () => {
  ...
  assert.deepEqual(
    (capturedProjectRequest as { extra_tools: string[] }).extra_tools,
    ["web_search", "browser_get_state", "plugin_workflow_create"],
  );
});

test("runTsRunnerCli keeps workspace app runtime tools for General-executor subagents", async () => {
...
test("runTsRunnerCli exposes record inquiry runtime tools only to the General subagent", async () => {
  setTempSandboxRoot("hb-ts-runner-general-record-runtime-tools-");
  let capturedProjectRequest: Record<string, unknown> | null = null;
        ...
        ...testDeps({
          pluginOverrides: {
            stageRuntimeTools: () => ({
              changed: false,
              toolIds: [
                "records_get_retrace_context",
                "plugin_workflow_list_revisions",
                "plugin_workflow_get_revision",
                "plugin_workflow_get",
              ],
            }),
          },
...
test("runTsRunnerCli keeps workspace modeling runtime tools for ordinary subagents", async () => {
  setTempSandboxRoot("hb-ts-runner-general-no-modeling-runtime-tools-");
  let capturedProjectRequest: Record<string, unknown> | null = null;
          ...
          pluginOverrides: {
            stageRuntimeTools: () => ({
              ...
              toolIds: [
                ...
                "base_object_define",
                // Intentionally NOT exercising "cronjobs_create" here:
                // it's listed in SUBAGENT_BLOCKED_RUNTIME_TOOL_IDS so
                // subagents never receive it (workspace cronjobs are a
                // front-of-house controller concern, not a backstage
                // executor one). This test covers workspace MODELING
                // tools.
                "plugin_workflow_create",
              ],
            }),
...
test("runTsRunnerCli keeps workspace app runtime tools for non-specialized subagents", async () => {
...
test("runTsRunnerCli exposes embedded default skills to main-session pi harness requests", async () => {
  const sandboxRoot = fs.mkdtempSync(
...
test("runTsRunnerCli does not load App Builder specialist embedded skills for collapsed subagents", async () => {
  const sandboxRoot = fs.mkdtempSync(
