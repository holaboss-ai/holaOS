...
function buildMainSessionEventBatchInstruction(
  ...
  if (deliveryBucket === "waiting_on_user") {
    ...
    lines.push(
      ...
      "If completed work established clearly stable workspace-wide defaults that future runs should obey by default, use `update_workspace_instructions` to record them in `AGENTS.md` before replying, but only when they are stable, likely to recur, or explicitly confirmed as future defaults.",
      "If a possible `AGENTS.md` update still needs confirmation about becoming the default for future runs, ask before persisting it.",
      "Use `AGENTS.md` for rules, defaults, conventions, and recurring commands that should shape behavior by default; keep named-subject facts, one-off deliverables, unresolved hypotheses, partial investigations, and temporary runtime state out of it.",
      "If an event comes from a scheduled workflow or a legacy cronjob compatibility event, treat it like a specific automation update rather than a generic status bulletin.",
