...
test("persistIntegrationCandidate reuses the same workspace tree when owner provenance changes", async () => {
  const root = makeTempDir("hb-integration-memory-workspace-identity-");
  const workspaceRoot = path.join(root, "workspace");
  const store = new RuntimeStateStore({
    dbPath: path.join(root, "runtime.db"),
    workspaceRoot,
  ...
  const first = await persistIntegrationCandidate({
    store,
    workspaceId: "workspace-1",
    candidate: {
      provider: "gmail",
      ...
      content: "# Launch follow-up\n\nCustomer thread is waiting on a follow-up.\n",
      tags: ["gmail"],
  ...
  const second = await persistIntegrationCandidate({
    store,
    workspaceId: "workspace-1",
    candidate: {
      provider: "gmail",
      accountNamespace: "ops@example.com",
      accountDisplayName: "Ops Gmail",
      subjectKey: "thread:launch-2",
      entityKey: "thread:launch-2",
      entityLabel: "Launch thread follow-up",
      branchKey: "threads",
      branchLabel: "Threads",
      title: "Launch reminder",
      summary: "A second thread still belongs to the same workspace account namespace.",
      content: "# Launch reminder\n\nA second thread still belongs to the same workspace account namespace.\n",
      tags: ["gmail"],
      sourceType: "gmail.thread",
      sourceEventId: "evt-2",
      sourceMessageId: "msg-2",
      externalObjectId: "thread-2",
      externalObjectType: "thread",
      confidence: 0.9,
      observedAt: "2026-06-02T01:00:00.000Z",
    },
    embeddingClient: null,
  });

  assert.equal(second.tree.treeId, first.tree.treeId);
  const trees = store.listSemanticMemoryNodes({
    category: "workspace",
    workspaceId: "workspace-1",
    treeId: first.tree.treeId,
    nodeClass: "semantic",
    nodeKind: "connection",
    status: "active",
    limit: 100,
  ...
  assert.equal(trees[0]?.metadata.account_namespace, "ops@example.com");
  assert.equal(trees[0]?.title, "Ops Gmail");

  store.close();
});

test("persistIntegrationCandidate detects semantic duplicates before legacy fingerprint lookup", async () => {
  const root = makeTempDir("hb-integration-memory-semantic-duplicate-");
  const workspaceRoot = path.join(root, "workspace");
  const store = new RuntimeStateStore({
    dbPath: path.join(root, "runtime.db"),
    workspaceRoot,
  ...
  try {
    ...
    const initial = await persistIntegrationCandidate({
      store,
      workspaceId: "workspace-1",
      candidate: {
        provider: "gmail",
    ...
    await rebuildIntegrationTree({
      store,
      workspaceId: "workspace-1",
      treeId: initial.tree.treeId,
      embeddingClient: null,
    });

    store.getIntegrationLeafByFingerprint = () => {
      throw new Error("legacy fingerprint lookup should not be used when semantic leaves exist");
    };

    const duplicate = await persistIntegrationCandidate({
      store,
      workspaceId: "workspace-1",
      candidate: {
        provider: "gmail",
    ...
    assert.equal(duplicate.outcome, "noop_duplicate");
    assert.equal(duplicate.leaf.leafId, initial.leaf.leafId);
  } finally {
    store.close();
  }
});
...
test.skip("retrieveIntegrationMemory adds vector-only semantic candidates that fall outside the recent doc window", async () => {
    ...
    store.replaceSemanticMemorySearchDocs({
      ...
      docs: [
        ...Array.from({ length: 170 }, (_, index) => ({
          ...
          path: index === 169
            ? "semantic/workspace/systems/integrations/github-vector-topk/repo-archive-ledger/content.md"
            : `semantic/workspace/systems/integrations/github-vector-topk/filler-${index + 1}/content.md`,
          childCount: 0,
...
test("retrieveIntegrationMemory surfaces integration memory without requiring integration bindings", async () => {
...
test("rebuildIntegrationTree writes the Notion semantic memory hierarchy", async () => {
  ...
  try {
    ...
    const rootNode = store.getSemanticMemoryNodeByPath({
      category: "workspace",
      workspaceId: "workspace-1",
      path: "semantic/workspace/systems/integrations/notion-product-ops-acct-1/content.md",
    });
    assert.ok(rootNode);
    ...
    store.deleteIntegrationTreeMemory({ workspaceId: "workspace-1", treeId });
    assert.equal(
      store.listSemanticMemoryNodes({
        category: "workspace",
        workspaceId: "workspace-1",
