...
test("runtime queue worker retries a restart-interrupted scheduled (cron) run instead of failing it", async () => {
  const root = makeTempDir("hb-runtime-queue-worker-scheduled-retry-");
  const store = new RuntimeStateStore({
    dbPath: path.join(root, "runtime.db"),
    workspaceRoot: path.join(root, "workspace")
  });
  ...
  const stale = store.enqueueInput({
    workspaceId: "workspace-1",
    sessionId: "scheduled-abc",
    payload: { text: "Say Hi", context: { cronjob_id: "cron-1" } }
  });
  store.claimInputs({
    limit: 1,
    claimedBy: "worker-old",
  ...
  // A cron run that began streaming before the runtime was restarted out from
  // under it: it has a run_started event but no terminal event / turn result.
  store.appendOutputEvent({
    workspaceId: "workspace-1",
    sessionId: "scheduled-abc",
    inputId: stale.inputId,
    sequence: 1,
    eventType: "run_claimed",
    payload: { instruction_preview: "Say Hi" },
  });
  store.appendOutputEvent({
    workspaceId: "workspace-1",
    sessionId: "scheduled-abc",
  ...
  await worker.processAvailableInputsOnce();
  ...
  // The interrupted cron run is retried (re-executed) rather than hard-failed.
  assert.deepEqual(seen, [stale.inputId]);
  assert.equal(updated?.status, "DONE");
  assert.equal(updated?.attempt, 1);
  assert.equal(events.some((event) => event.eventType === "run_failed"), false);
  // The stale partial events were cleared so the retry starts clean.
  assert.equal(events.some((event) => event.eventType === "run_started"), false);
