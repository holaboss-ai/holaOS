...
export type MemoryRerankerCategory = "workspace";
...
function isProviderBackedWorkspaceEvidence(evidence: BaseMemoryRerankerEvidence): boolean {
...
function directUserImpactScore(evidence: BaseMemoryRerankerEvidence): number {
  ...
  if (hasPattern(/(team|ops|manager|lead|approval)/i, evidence)) {
    score += 0.45;
  }
  return score;
}
...
function intentSpecificScore(params: {
  ...
  switch (intent) {
    ...
    case "briefing":
      ...
      if (isProviderBackedWorkspaceEvidence(evidence)) {
        score += 0.6;
      }
