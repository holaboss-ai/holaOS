...
export class RuntimeIntegrationService {
  ...
  upsertBinding(params: {
    ...
    // Composio accounts (ca_...) are remote-only in Stage 3 — never mirrored into
    // the local control-plane store, so a local lookup would 404. They're
    // validated at connect / enumeration time (and the caller only offers
    // accounts matching this provider), so trust the id here. Local (bot-token)
    // connections still get the existence + provider-match checks.
    if (!connectionId.trim().toLowerCase().startsWith("ca_")) {
  ...
  async checkReadiness(params: {
    ...
    try {
      ...
      return await checkIntegrationReadiness({
        store: this.store,
        workspaceId,
        appId,
