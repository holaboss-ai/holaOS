...
test("shared harness definitions expose pi + the CLI harnesses (Claude Code, Codex)", () => {
  // Snapshot of every registered harness's identity + the prep plan
  // they advertise for a generic main-session request. The CLI
  // harnesses (claude-code, codex) deliberately differ from pi on
  // `startWorkspaceMcpSidecar` — they wire MCP via per-CLI config
  // (--mcp-config / config.toml) rather than the workspace MCP sidecar
  // pi reuses.
  assert.deepEqual(
