...
test("compileWorkspaceRuntimePlan folds mcp_registry.app_servers into the resolved server set", () => {
  const plan = compileWorkspaceRuntimePlan({
    workspace_id: "workspace-app-servers",
    workspace_yaml: `
agents:
  id: workspace.general
  model: openai/gpt-5
mcp_registry:
  ...
  servers:
    adspower:
      type: remote
      url: "https://adspower.example/mcp"
      enabled: true
  app_servers:
    helm:
      type: remote
      url: "https://api.example.com/mcp/helm/sse"
      enabled: true
      owner_app_id: helm
`,
    references: {}
  ...
  // Both the standalone-pool server and the app-container-owned server compile
  // into the agent's resolved set — the agent still gets every tool.
  const ids = plan.resolved_mcp_servers.map((server) => server.server_id);
  assert.ok(ids.includes("adspower"), "standalone pool server present");
  assert.ok(ids.includes("helm"), "app_servers server folded in");
});
