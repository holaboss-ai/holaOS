 ...
 *   jobs                    Queue worker / cron / post-run job snapshot
...
import { controlPlaneDbPath, runtimeDbPath } from "./store.js";
...
const commands: Record<string, (ctx: CommandContext) => Promise<number> | number> = {
  ...
  jobs: ({ db, args, out }) => {
    ...
    // Single-tenant end-state: every live workspace's runtime data now lives in
    // the one root data.db (sibling of control-plane.db), not in per-workspace
    // runtime.db files. Read that once for the consolidated queue/cron/post-run
    // snapshot. The legacy host-state cronjobs/inputs/jobs tables are merged
    // below for any workspace NOT yet folded into the root (no `projects` row).
    const workspaceIdsWithRuntimeDb = new Set<string>();
    const rootRuntimeDbPath = path.join(
      path.dirname(controlPlaneDbPath({ hostStateDbPath: args.dbPath })),
      "data.db",
    );
    if (existsSync(rootRuntimeDbPath)) {
      const rootDb = openDb(rootRuntimeDbPath);
      try {
        const rootQueueRows = safeAll(
          rootDb,
          `SELECT status, COUNT(*) AS count FROM agent_session_inputs GROUP BY status`,
        );
        if (Array.isArray(rootQueueRows)) {
          for (const row of rootQueueRows as Array<{ status?: string; count?: number }>) {
            const key = typeof row.status === "string" ? row.status : "";
            queueCounts.set(key, (queueCounts.get(key) ?? 0) + Number(row.count ?? 0));
          }
        }
        const rootPostRunRows = safeAll(
          rootDb,
          `SELECT status, COUNT(*) AS count FROM post_run_jobs GROUP BY status`,
        );
        if (Array.isArray(rootPostRunRows)) {
          for (const row of rootPostRunRows as Array<{ status?: string; count?: number }>) {
            const key = typeof row.status === "string" ? row.status : "";
            postRunCounts.set(key, (postRunCounts.get(key) ?? 0) + Number(row.count ?? 0));
          }
        }
        const rootCronRows = safeAll(
          rootDb,
          `SELECT enabled, COUNT(*) AS count FROM cronjobs GROUP BY enabled`,
        );
        if (Array.isArray(rootCronRows)) {
          for (const row of rootCronRows as Array<{ enabled?: number; count?: number }>) {
            const key = String(row.enabled ?? 0);
            cronCounts.set(key, (cronCounts.get(key) ?? 0) + Number(row.count ?? 0));
          }
        }
        // Workspaces folded into the root register a `projects` row. Their legacy
        // host-state rows are already counted above (consolidation copied them),
        // so exclude them from the legacy merge to avoid double-counting.
        const consolidatedRows = safeAll(
          rootDb,
          `SELECT project_id FROM projects`,
        );
        if (Array.isArray(consolidatedRows)) {
          for (const row of consolidatedRows as Array<{ project_id?: string }>) {
            if (typeof row.project_id === "string" && row.project_id) {
              workspaceIdsWithRuntimeDb.add(row.project_id);
            }
          }
        }
      } finally {
...
Commands:
  ...
  jobs                    Queue / cron / post-run job snapshot
