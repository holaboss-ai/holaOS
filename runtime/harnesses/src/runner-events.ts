...
export function summarizeHarnessQuestionPrompt(args: unknown, result: unknown): string | null {
  // ask_user_question wraps the prompt as `{question: {prompt, options}}`
  // or `{question: {questions: [{prompt, ...}, ...]}}`. Unwrap one
  // level so the summarizer keeps working alongside the older shape
  // where `question` was a plain string.
  const unwrapStructuredQuestion = (value: unknown): unknown[] => {
    if (!isRecord(value)) return [value];
    const collected: unknown[] = [value];
    collected.push(value.prompt, value.message, value.text, value.content, value.title);
    if (Array.isArray(value.questions) && value.questions.length > 0) {
      const first = value.questions[0];
      if (isRecord(first)) {
        collected.push(first.prompt, first.message, first.text, first.content, first.title);
      }
    }
    return collected;
  };
  ...
  if (isRecord(args)) {
    candidates.push(...unwrapStructuredQuestion(args.question));
    candidates.push(args.prompt, args.message, args.text, args.content);
  }
  if (isRecord(result)) {
    candidates.push(...unwrapStructuredQuestion(result.question));
    candidates.push(result.prompt, result.message, result.text, result.content);
    if (isRecord(result.details)) {
      candidates.push(...unwrapStructuredQuestion(result.details.question));
      candidates.push(
