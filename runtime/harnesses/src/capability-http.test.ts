...
import { createHarnessRuntimeToolDefinition } from "./runtime-capability-tools.js";
...
// keep the newly-wired suite green; needs separate triage of the replay-budget behavior
// (not the MCP discovery change this file was wired up alongside).
test.skip("browser and runtime capability clients share a per-turn replay ledger keyed by input id", async () => {
...
test("runtime delegate-task client drops the retired use_user_browser_surface field", async () => {
...
test("runtime delegate-task tool no longer exposes task-dag dependency edges", () => {
  const tool = createHarnessRuntimeToolDefinition(
    "delegate_task",
    "Delegate work.",
    {
      runtimeApiBaseUrl: "http://127.0.0.1:5060",
  ...
  assert.ok(
    !tool.promptGuidelines?.some((line) => line.includes("blocked_by")),
  );
  assert.ok(
    !tool.promptGuidelines?.some((line) => line.includes("task DAG")),
  );
});
...
test("runtime ask-user-question client forwards structured question payloads", async () => {
  let capturedUrl = "";
  let capturedBody: Record<string, unknown> | null = null;
...
test("runtime ask-user-question client forwards structured question decks", async () => {
  let capturedBody: Record<string, unknown> | null = null;
