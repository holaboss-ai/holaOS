...
test("ask_user_question tool flips waitingForUser when call succeeds", () => {
...
test("ask_user_question tool error does not flip waitingForUser", () => {
  const state = freshState();
  noteHarnessWaitingForUserOnToolCompletion({
    toolName: "ask_user_question",
    isError: true,
...
test("non-ask_user_question tool with no result does not flip", () => {
  const state = freshState();
  noteHarnessWaitingForUserOnToolCompletion({
