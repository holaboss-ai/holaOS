...
const DEFAULT_FALLBACK_CONTEXT_WINDOW = 500_000;
const DEFAULT_FALLBACK_MAX_TOKENS = 128_000;
const UNIFORM_MODEL_MAX_TOKENS = 128_000;
let cachedRuntimeConfigModelCatalogPath: string | null = null;
let cachedRuntimeConfigModelCatalogMtimeMs: number | null = null;
let cachedRuntimeConfigModelCatalog:
  | Record<string, Record<string, HarnessCatalogModelEntry>>
  | null = null;

function emptyHarnessModelCatalog(): Record<
  string,
...
function runtimeConfigCatalogEntryFromModelPayload(
  token: string,
  rawModel: unknown,
): { providerId: string; modelId: string; entry: HarnessCatalogModelEntry } | null {
  if (!isRecord(rawModel)) {
    return null;
  }

  let providerId =
    nonEmptyStringValue(rawModel.provider_id) ??
    nonEmptyStringValue(rawModel.provider);
  let modelId =
    nonEmptyStringValue(rawModel.model_id) ??
    nonEmptyStringValue(rawModel.model);

  if ((!providerId || !modelId) && token.includes("/")) {
    const [tokenProviderId, ...rest] = token.split("/");
    if (!providerId) {
      providerId = tokenProviderId.trim() || null;
    }
    if (!modelId && rest.length > 0) {
      modelId = rest.join("/").trim() || null;
    }
  }
  ...
  const entry: HarnessCatalogModelEntry = {
    contextWindow: rawModel.contextWindow ?? rawModel.context_window,
    maxTokens: rawModel.maxTokens ?? rawModel.max_tokens,
  };
  if (isRecord(rawModel.cost)) {
    entry.cost = {
      input: rawModel.cost.input,
      output: rawModel.cost.output,
      cacheRead: rawModel.cost.cacheRead,
      cacheWrite: rawModel.cost.cacheWrite,
    };
  }
  return {
...
export function runtimeConfigModelCatalog(): Record<
  string,
  Record<string, HarnessCatalogModelEntry>
> {
  ...
  if (!configPath) {
    ...
    cachedRuntimeConfigModelCatalog = null;
    return emptyHarnessModelCatalog();
  }

  try {
    const stats = fs.statSync(configPath);
    if (
      cachedRuntimeConfigModelCatalog &&
      cachedRuntimeConfigModelCatalogPath === configPath &&
      cachedRuntimeConfigModelCatalogMtimeMs === stats.mtimeMs
    ) {
      return cachedRuntimeConfigModelCatalog;
    }

    const rawText = fs.readFileSync(configPath, "utf8");
    const parsed = JSON.parse(rawText);
    const document = isRecord(parsed) ? parsed : {};
    const modelsPayload = isRecord(document.models) ? document.models : {};
    const catalog = emptyHarnessModelCatalog();

    for (const [token, rawModel] of Object.entries(modelsPayload)) {
      const normalized = runtimeConfigCatalogEntryFromModelPayload(token, rawModel);
      if (!normalized) {
        continue;
      }
      const providerBucket =
        catalog[normalized.providerId] ??
        (catalog[normalized.providerId] = Object.create(null) as Record<
          string,
          HarnessCatalogModelEntry
        >);
      providerBucket[normalized.modelId] = {
        ...(providerBucket[normalized.modelId] ?? {}),
        ...normalized.entry,
      };
    }
    ...
    cachedRuntimeConfigModelCatalogPath = configPath;
    cachedRuntimeConfigModelCatalogMtimeMs = stats.mtimeMs;
    cachedRuntimeConfigModelCatalog = catalog;
    return catalog;
  } catch {
    cachedRuntimeConfigModelCatalogPath = configPath;
    cachedRuntimeConfigModelCatalogMtimeMs = null;
    cachedRuntimeConfigModelCatalog = emptyHarnessModelCatalog();
    return cachedRuntimeConfigModelCatalog;
  }
}

export function mergeHarnessModelCatalogs(
  ...catalogs: Array<
    Record<string, Record<string, HarnessCatalogModelEntry>> | null | undefined
  >
): Record<string, Record<string, HarnessCatalogModelEntry>> {
  const merged = emptyHarnessModelCatalog();
  for (const catalog of catalogs) {
    ...
    for (const [providerId, models] of Object.entries(catalog)) {
      const providerBucket =
        merged[providerId] ??
        (merged[providerId] = Object.create(null) as Record<
          string,
          HarnessCatalogModelEntry
        >);
      for (const [modelId, entry] of Object.entries(models)) {
        providerBucket[modelId] = {
          ...(providerBucket[modelId] ?? {}),
          ...entry,
        };
      }
...
function openAiCompatForRequest(request: HarnessModelRoutingRequest): HarnessOpenAiCompat | undefined {
  ...
  if (modelProxyProvider === "openai_compatible" && modelId.startsWith("qwen/")) {
    // OpenRouter's Qwen chat/completions path rejects the OpenAI `developer`
    // role even when reasoning is enabled, so force the prompt back to
    // `system` while keeping the model on the reasoning-capable path.
    return {
      supportsDeveloperRole: false,
    };
  }
...
function knownModelBudgetOverride(
  request: Pick<HarnessModelRoutingRequest, "model_id" | "provider_id" | "model_client">,
  api: HarnessModelApi,
): HarnessModelBudget | null {
  const normalizedModelId = normalizeHarnessModelId(request.model_id);
  const normalizedProviderId = request.provider_id.trim().toLowerCase();
  const normalizedProxyProvider =
    request.model_client.model_proxy_provider.trim().toLowerCase();
  // Keep legacy Codex budget sizing for persisted snapshots even though
  // execution stays on the generic OpenAI-compatible path.
  const shouldUseLegacyCodexBudgetOverride =
    normalizedProviderId === "openai_codex" &&
    normalizedProxyProvider === "openai_compatible";
  if (shouldUseLegacyCodexBudgetOverride) {
  ...
  const maxTokens =
    ...
    entry.maxTokens > 0
      ...
      : typeof entry?.max_tokens === "number" &&
          Number.isFinite(entry.max_tokens) &&
          entry.max_tokens > 0
        ? entry.max_tokens
        : null;
  if (
