...
export const DESKTOP_BROWSER_TOOL_DEFINITIONS: DesktopBrowserToolDefinition[] = [
  {
    id: "browser_navigate",
    description:
      "Navigate a browser profile to a URL for direct inspection or interaction on a live site when search results are not enough. Optional `browser_profile`: which of the user's Browser Profiles to drive — its name, its number (e.g. \"#1\" from browser_list_profiles), or its id. Pass it when the user named or clearly implied a specific browser; OMIT it to auto-pick the user's pinned default, else a currently-open browser, else the only/first profile. Never use a third-party tool like AdsPower. The chosen profile becomes the active target for the follow-up browser tools this session.",
    policy: "mutate",
  ...
  },
  {
    id: "browser_open_tab",
    description:
      "Open a URL in a new tab of a browser profile so you can inspect or compare live pages without losing the current page state. Optional `browser_profile`: which Browser Profile to drive — its name, its number (e.g. \"#1\"), or its id. Omit it to auto-pick the user's pinned default, else a currently-open browser.",
    policy: "mutate",
    session_scope: "all_sessions",
    input_schema: {
      type: "object",
