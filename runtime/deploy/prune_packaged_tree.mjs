...
const TREE_SITTER_WASMS_TO_KEEP = new Set([
...
function pruneTreeSitterWasms(rootPath) {
  for (const entry of readdirSync(rootPath, { withFileTypes: true })) {
    const entryPath = path.join(rootPath, entry.name);
    if (!entry.isDirectory()) {
      continue;
    ...
    if (
      entry.name === "out" &&
      path.basename(rootPath) === "tree-sitter-wasms" &&
      path.basename(path.dirname(rootPath)) === "node_modules"
    ) {
      for (const wasmEntry of readdirSync(entryPath, { withFileTypes: true })) {
        if (!wasmEntry.isFile() || !wasmEntry.name.endsWith(".wasm")) {
          continue;
        }
        if (TREE_SITTER_WASMS_TO_KEEP.has(wasmEntry.name)) {
          continue;
        }
        rmSync(path.join(entryPath, wasmEntry.name), { force: true });
      }
      continue;
    }

    pruneTreeSitterWasms(entryPath);
  }
}
...
export function prunePackagedTree(targetRoot, targetPlatform = "", targetArch = process.arch) {
  ...
  pruneTreeSitterWasms(resolvedRoot);
  if (targetPlatform) {
...
if (isDirectRun()) {
  ...
  if (!targetRoot) {
    console.error("usage: prune_packaged_tree.mjs <target-root> [platform] [arch]");
    process.exit(1);
  }
  prunePackagedTree(targetRoot, targetPlatform, targetArch);
}
